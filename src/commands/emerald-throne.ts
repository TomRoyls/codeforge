...
} from './emerald-throne-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './emerald-throne-format-helpers.js'

/** @example codeforge emerald-throne ./src */
export default class EmeraldThrone extends Command {
  ...
  /** @example EmeraldThrone.run() */
  public async run(): Promise<EmeraldThroneResult> {
    const { args, flags } = await this.parse(EmeraldThrone)
    ...
    try {
      ...
      if (flags.verbose) {
        this.log(chalk.dim(`\nAnalyzed ${files.length} files across ${result.courts.length} courts`))
      }
