...
import {
  ...
  gatherFiles,
} from './platinum-crown-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './platinum-crown-format-helpers.js'

/**
 * @example
 * codeforge platinum-crown ./src
 */
export default class PlatinumCrown extends Command {
  static override description = 'Analyze code royal-purity, crest-authority, jewel-precision, circlet-resilience, and reign-endurance'
  ...
  async run(): Promise<void> {
    const { args, flags } = await this.parse(PlatinumCrown)
    const spinner = ora('Scanning platinum crown...').start()
    ...
    try {
      ...
      spinner.text = 'Computing platinum crown analysis...'
      const result = await buildPlatinumCrownResult(
        Array.from(files),
        contents,
        { ext: flags.ext, verbose: flags.verbose },
      )
      spinner.succeed('Platinum crown analysis complete')
      ...
      spinner.fail('Platinum crown analysis failed')
