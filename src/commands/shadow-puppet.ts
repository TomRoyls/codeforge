...
import { buildShadowPuppetResult, gatherFiles } from './shadow-puppet-helpers.js'
import { formatResultTable, formatResultJson } from './shadow-puppet-format-helpers.js'

// ─── Command ───────────────────────────────────────────────────────

/**
 * Analyze code like shadow puppet theater
 * @example
 * codeforge shadow-puppet ./src
 */
export default class ShadowPuppet extends Command {
  static override description = 'Analyze code silhouette clarity, projection quality, limb articulation, screen presence, and narrative flow'
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      const result = await buildShadowPuppetResult(files, contents, { verbose: flags.verbose })
      spinner.succeed(`Analyzed ${result.stats.totalFiles} file(s) across ${result.stats.totalTheaters} theater(s)`)
      ...
      if (flags.verbose) {
        this.log(`\nPuppeteer Grade: ${result.stats.puppeteerGrade}`)
        this.log(`Overall Artistry: ${result.stats.overallArtistry}`)
      }
