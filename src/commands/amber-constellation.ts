...
import {
  ...
  gatherFiles,
} from './amber-constellation-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './amber-constellation-format-helpers.js'

/**
 * @example
 * codeforge amber-constellation ./src
 */
export default class AmberConstellation extends Command {
  static override description = 'Analyze code preservation-quality, stellar-organization, star-clarity, constellation-coherence, and cosmic-depth'
  ...
  async run(): Promise<void> {
    const { args, flags } = await this.parse(AmberConstellation)
    const spinner = ora('Scanning amber constellation...').start()
    ...
    try {
      ...
      spinner.text = 'Computing amber constellation analysis...'
      const result = await buildAmberConstellationResult(
        Array.from(files),
        contents,
        { ext: flags.ext, verbose: flags.verbose },
      )
      spinner.succeed('Amber constellation analysis complete')
      ...
      spinner.fail('Amber constellation analysis failed')
