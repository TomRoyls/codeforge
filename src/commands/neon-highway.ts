...
import { buildNeonHighwayResult, gatherFiles } from './neon-highway-helpers.js'
import { formatResultTable, formatResultJson } from './neon-highway-format-helpers.js'

// ─── Command ───────────────────────────────────────────────────────

/**
 * Analyze code luminosity, traffic flow, lane discipline, signal quality, and route efficiency
 * @example
 * codeforge neon-highway ./src
 */
export default class NeonHighway extends Command {
  static override description = 'Analyze code luminosity, traffic flow, lane discipline, signal quality, and route efficiency'
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      const result = await buildNeonHighwayResult(files, contents, { verbose: flags.verbose })
      spinner.succeed(`Analyzed ${result.stats.totalFiles} file(s) across ${result.stats.totalSystems} system(s)`)
      ...
      if (flags.verbose) {
        ...
        this.log(`Overall Performance: ${result.stats.overallPerformance}`)
      }
