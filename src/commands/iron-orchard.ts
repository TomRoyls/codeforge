...
import {
  ...
  type IronGardenResult,
} from './iron-orchard-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './iron-orchard-format-helpers.js'

/** @example codeforge iron-orchard ./src */
export default class IronOrchard extends Command {
  static override args = {
    path: Args.string({ default: '.', description: 'Path to analyze' }),
  }

  static override description = 'Iron garden analysis — strength-through-nature/rust-resistance/bloom-precision/root-depth/forge-vitality'
  ...
  /** @example IronOrchard.run() */
  public async run(): Promise<IronGardenResult> {
    const { args, flags } = await this.parse(IronOrchard)
    const targetPath = args.path as string
    const spinner = ora('Analyzing iron garden…').start()
    ...
    try {
      ...
      spinner.succeed('Iron garden analysis complete')
      ...
      spinner.fail('Iron garden analysis failed')
      throw error
    }
  }
}
