...
import {
  ...
  gatherFiles,
} from './thunder-forge-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './thunder-forge-format-helpers.js'

/**
 * @example
 * codeforge thunder-forge ./src
 */
export default class ThunderForge extends Command {
  static override description = 'Analyze code lightning-power, anvil-strength, spark-quality, temper-precision, and storm-craft'
  ...
  async run(): Promise<void> {
    const { args, flags } = await this.parse(ThunderForge)
    const spinner = ora('Scanning thunder forge...').start()
    ...
    try {
      ...
      spinner.text = 'Computing thunder forge analysis...'
      const result = await buildThunderForgeResult(
        Array.from(files),
        contents,
        { ext: flags.ext, verbose: flags.verbose },
      )
      spinner.succeed('Thunder forge analysis complete')
      ...
      spinner.fail('Thunder forge analysis failed')
