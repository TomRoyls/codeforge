...
import {
  ...
  gatherFiles,
} from './frost-sanctuary-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './frost-sanctuary-format-helpers.js'

/**
 * @example
 * codeforge frost-sanctuary ./src
 */
export default class FrostSanctuary extends Command {
  static override description = 'Analyze code crystal-preservation, ice-clarity, permafrost-stability, frost-resilience, and aurora-beauty'
  ...
  async run(): Promise<void> {
    const { args, flags } = await this.parse(FrostSanctuary)
    const spinner = ora('Scanning frost sanctuary...').start()
    ...
    try {
      ...
      spinner.text = 'Computing frost sanctuary analysis...'
      const result = await buildFrostSanctuaryResult(
        Array.from(files),
        contents,
        { ext: flags.ext, verbose: flags.verbose },
      )
      spinner.succeed('Frost sanctuary analysis complete')
      ...
      spinner.fail('Frost sanctuary analysis failed')
