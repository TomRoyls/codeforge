...
} from './copper-anvil-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './copper-anvil-format-helpers.js'

/** @example codeforge copper-anvil ./src */
export default class CopperAnvil extends Command {
  static override args = {
    path: Args.string({ default: '.', description: 'Path to analyze' }),
  }

  static override description = 'Bronze anvil analysis — alloy-strength/patina-wisdom/dawn-clarity/forge-precision/durable-current'
  ...
  /** @example CopperAnvil.run() */
  public async run(): Promise<BronzeAnvilResult> {
    const { args, flags } = await this.parse(CopperAnvil)
    const targetPath = args.path as string
    const spinner = ora('Analyzing bronze anvil…').start()
    ...
    try {
      ...
      const result = await buildBronzeAnvilResult(files, contents)

      spinner.succeed('Bronze anvil analysis complete')
      ...
      if (flags.verbose) {
        this.log(chalk.dim(`\nAnalyzed ${files.length} files across ${result.workshops.length} workshops`))
      }

      return result
    } catch (error: unknown) {
      spinner.fail('Bronze anvil analysis failed')
      throw error
    }
  }
}
