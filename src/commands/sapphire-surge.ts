...
} from './sapphire-surge-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './sapphire-surge-format-helpers.js'

/** @example codeforge sapphire-surge ./src */
export default class SapphireSurge extends Command {
  static override args = {
    path: Args.string({ default: '.', description: 'Path to analyze' }),
  }

  static override description = 'Sapphire tide analysis — gem-depth/tidal-rhythm/wave-purity/ocean-wisdom/tide-resilience'
  ...
  /** @example SapphireSurge.run() */
  public async run(): Promise<SapphireTideResult> {
    const { args, flags } = await this.parse(SapphireSurge)
