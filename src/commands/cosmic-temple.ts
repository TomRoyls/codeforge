...
} from './cosmic-temple-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './cosmic-temple-format-helpers.js'

/** @example codeforge cosmic-temple ./src */
export default class CosmicTemple extends Command {
  static override args = {
    path: Args.string({ default: '.', description: 'Path to analyze' }),
  }

  static override description = 'Stellar cathedral analysis — cosmic-architecture/star-sanctity/vault-clarity/celestial-precision/dawn-transcendence'
  ...
  /** @example CosmicTemple.run() */
  public async run(): Promise<StellarCathedralResult> {
    const { args, flags } = await this.parse(CosmicTemple)
    const targetPath = args.path as string
    const spinner = ora('Analyzing stellar cathedral…').start()
    ...
    try {
      ...
      const result = await buildStellarCathedralResult(files, contents)

      spinner.succeed('Stellar cathedral analysis complete')
      ...
      spinner.fail('Stellar cathedral analysis failed')
      throw error
    }
  }
}
