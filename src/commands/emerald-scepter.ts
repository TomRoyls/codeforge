...
} from './emerald-scepter-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './emerald-scepter-format-helpers.js'

/** @example codeforge emerald-scepter ./src */
export default class EmeraldScepter extends Command {
  ...
  /** @example EmeraldScepter.run() */
  public async run(): Promise<EmeraldThroneResult> {
    const { args, flags } = await this.parse(EmeraldScepter)
