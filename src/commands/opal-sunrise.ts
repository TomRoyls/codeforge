...
import {
  ...
  gatherFiles,
} from './opal-sunrise-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './opal-sunrise-format-helpers.js'

/**
 * @example
 * codeforge opal-sunrise ./src
 */
export default class OpalSunrise extends Command {
  static override description = 'Analyze code play-of-color, dawn-clarity, fire-warmth, spectrum-richness, and opalescence-quality'
  ...
  async run(): Promise<void> {
    const { args, flags } = await this.parse(OpalSunrise)
    const spinner = ora('Scanning opal sunrise...').start()
    ...
    try {
      ...
      spinner.text = 'Computing opal sunrise analysis...'
      const result = await buildOpalSunriseResult(
        Array.from(files),
        contents,
        { ext: flags.ext, verbose: flags.verbose },
      )
      spinner.succeed('Opal sunrise analysis complete')
      ...
      spinner.fail('Opal sunrise analysis failed')
