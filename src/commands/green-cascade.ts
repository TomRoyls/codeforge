...
} from './green-cascade-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './green-cascade-format-helpers.js'

/** @example codeforge jade-cascade ./src */
export default class GreenCascade extends Command {
  static override args = {
    path: Args.string({ default: '.', description: 'Path to analyze' }),
  }

  static override description = 'Jade cascade analysis — flow-grace/cascade-clarity/pool-depth/mist-purity/river-wisdom'
  ...
  /** @example GreenCascade.run() */
  public async run(): Promise<JadeCascadeResult> {
    const { args, flags } = await this.parse(GreenCascade)
    const targetPath = args.path as string
    const spinner = ora('Analyzing jade cascade…').start()
    ...
    try {
      ...
      const result = await buildJadeCascadeResult(files, contents)

      spinner.succeed('Jade cascade analysis complete')
      ...
      if (flags.verbose) {
        this.log(chalk.dim(`\nAnalyzed ${files.length} files across ${result.streams.length} streams`))
      }

      return result
    } catch (error: unknown) {
      spinner.fail('Jade cascade analysis failed')
      throw error
    }
  }
}
