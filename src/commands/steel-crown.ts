...
} from './steel-crown-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './steel-crown-format-helpers.js'

/** @example codeforge iron-crown ./src */
export default class SteelCrown extends Command {
  static override args = {
    path: Args.string({ default: '.', description: 'Path to analyze' }),
  }

  static override description = 'Iron crown analysis — sovereign-strength/crown-authority/jewel-precision/circlet-resilience/reign-endurance'
  ...
  /** @example SteelCrown.run() */
  public async run(): Promise<IronCrownResult> {
    const { args, flags } = await this.parse(SteelCrown)
    const targetPath = args.path as string
    const spinner = ora('Analyzing iron crown…').start()
    ...
    try {
      ...
      spinner.succeed('Iron crown analysis complete')
      ...
      if (flags.verbose) {
        this.log(chalk.dim(`\nAnalyzed ${files.length} files across ${result.realms.length} realms`))
      }

      return result
    } catch (error: unknown) {
      spinner.fail('Iron crown analysis failed')
      throw error
    }
  }
}
