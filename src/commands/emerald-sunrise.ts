...
} from './emerald-sunrise-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './emerald-sunrise-format-helpers.js'

/** @example codeforge emerald-sunrise ./src */
export default class EmeraldSunrise extends Command {
  static override args = {
    path: Args.string({ default: '.', description: 'Path to analyze' }),
  }

  static override description = 'Emerald sunrise analysis — green-vitality/dawn-clarity/gem-wisdom/morning-freshness/sunrise-resilience'
  ...
  /** @example EmeraldSunrise.run() */
  public async run(): Promise<EmeraldSunriseResult> {
    const { args, flags } = await this.parse(EmeraldSunrise)
    const targetPath = args.path as string
    const spinner = ora('Analyzing emerald sunrise…').start()
    ...
    try {
      ...
      const result = await buildEmeraldSunriseResult(files, contents)

      spinner.succeed('Emerald sunrise analysis complete')
      ...
      spinner.fail('Emerald sunrise analysis failed')
      throw error
    }
  }
}
