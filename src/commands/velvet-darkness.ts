...
} from './velvet-darkness-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './velvet-darkness-format-helpers.js'

/** @example codeforge velvet-darkness ./src */
export default class VelvetDarkness extends Command {
  static override args = {
    path: Args.string({ default: '.', description: 'Path to analyze' }),
  }

  static override description = 'Velvet darkness analysis — softness-quality/dark-comfort/shadow-elegance/nocturnal-wisdom/night-resilience'
  ...
  /** @example VelvetDarkness.run() */
  public async run(): Promise<VelvetDarknessResult> {
    const { args, flags } = await this.parse(VelvetDarkness)
    const targetPath = args.path as string
    const spinner = ora('Analyzing velvet darkness…').start()
    ...
    try {
      ...
      const result = await buildVelvetDarknessResult(files, contents)

      spinner.succeed('Velvet darkness analysis complete')
      ...
      spinner.fail('Velvet darkness analysis failed')
      throw error
    }
  }
}
