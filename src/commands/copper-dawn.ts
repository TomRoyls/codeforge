...
import {
  ...
  gatherFiles,
} from './copper-dawn-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './copper-dawn-format-helpers.js'

/**
 * @example
 * codeforge copper-dawn ./src
 */
export default class CopperDawn extends Command {
  ...
  async run(): Promise<void> {
    const { args, flags } = await this.parse(CopperDawn)
    const spinner = ora('Scanning copper dawn...').start()
    ...
    try {
      ...
      spinner.text = 'Computing copper dawn analysis...'
      const result = await buildCopperDawnResult(
        Array.from(files),
        contents,
        { ext: flags.ext, verbose: flags.verbose },
      )
      spinner.succeed('Copper dawn analysis complete')
      ...
      spinner.fail('Copper dawn analysis failed')
