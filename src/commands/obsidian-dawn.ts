...
import {
  ...
  gatherFiles,
} from './obsidian-dawn-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './obsidian-dawn-format-helpers.js'

/**
 * @example
 * codeforge obsidian-dawn ./src
 */
export default class ObsidianDawn extends Command {
  static override description = 'Analyze code volcanic-clarity, edge-sharpness, mirror-depth, dark-wisdom, and dawn-emergence'
  ...
  async run(): Promise<void> {
    const { args, flags } = await this.parse(ObsidianDawn)
    const spinner = ora('Scanning obsidian dawn...').start()
    ...
    try {
      ...
      spinner.text = 'Computing obsidian dawn analysis...'
      const result = await buildObsidianDawnResult(
        Array.from(files),
        contents,
        { ext: flags.ext, verbose: flags.verbose },
      )
      spinner.succeed('Obsidian dawn analysis complete')
      ...
      spinner.fail('Obsidian dawn analysis failed')
