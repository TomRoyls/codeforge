...
} from './opal-horizon-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './opal-horizon-format-helpers.js'

/** @example codeforge opal-horizon ./src */
export default class OpalHorizon extends Command {
  ...
  /** @example OpalHorizon.run() */
  public async run(): Promise<OpalHorizonResult> {
    const { args, flags } = await this.parse(OpalHorizon)
    ...
    try {
      ...
      if (flags.verbose) {
        this.log(chalk.dim(`\nAnalyzed ${files.length} files across ${result.deposits.length} deposits`))
      }
