...
import {
  ...
  type OnyxCathedralResult,
} from './onyx-cathedral-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './onyx-cathedral-format-helpers.js'

/** @example codeforge onyx-cathedral ./src */
export default class OnyxCathedral extends Command {
  static override args = {
    path: Args.string({ default: '.', description: 'Path to analyze' }),
  }

  static override description = 'Onyx cathedral analysis — obsidian-clarity/dark-resilience/mirror-depth/midnight-precision/void-wisdom'
  ...
  /** @example OnyxCathedral.run() */
  public async run(): Promise<OnyxCathedralResult> {
    const { args, flags } = await this.parse(OnyxCathedral)
    const targetPath = args.path as string
    const spinner = ora('Analyzing onyx cathedral…').start()
    ...
    try {
      ...
      const result = await buildOnyxCathedralResult(files, contents)

      spinner.succeed('Onyx cathedral analysis complete')
      ...
      spinner.fail('Onyx cathedral analysis failed')
      throw error
    }
  }
}
