...
} from './quartz-vein-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './quartz-vein-format-helpers.js'

/** @example codeforge quartz-meridian ./src */
export default class QuartzVein extends Command {
  ...
  /** @example QuartzVein.run() */
  public async run(): Promise<QuartzMeridianResult> {
    const { args, flags } = await this.parse(QuartzVein)
    ...
    try {
      ...
      if (flags.verbose) {
        this.log(chalk.dim(`\nAnalyzed ${files.length} files across ${result.strata.length} strata`))
      }
