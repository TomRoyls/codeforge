...
import { buildObservatoryResult } from './observatory-helpers.js'
import { formatObservatoryResult, formatObservatoryJson } from './observatory-format-helpers.js'

export default class Observatory extends Command {
  static override description = 'Observe the codebase like an astronomer observes the night sky'
  ...
  async run(): Promise<void> {
    const { args, flags } = await this.parse(Observatory)
    ...
    try {
      ...
      const result = buildObservatoryResult(files, contents, flags)

      spinner.succeed(`Observatory scan complete — ${result.stats.totalBodies} bodies mapped`)

      const output = flags.format === 'json'
        ? formatObservatoryJson(result)
        : formatObservatoryResult(result)
      ...
      spinner.fail('Observatory scan failed')
