...
} from './ruby-pinnacle-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './ruby-pinnacle-format-helpers.js'

/** @example codeforge ruby-pinnacle ./src */
export default class RubyPinnacle extends Command {
  static override args = {
    path: Args.string({ default: '.', description: 'Path to analyze' }),
  }

  static override description = 'Ruby pinnacle analysis — crimson-vitality/peak-elegance/flame-precision/summit-resilience/crown-wisdom'
  ...
  /** @example RubyPinnacle.run() */
  public async run(): Promise<RubyPinnacleResult> {
    const { args, flags } = await this.parse(RubyPinnacle)
    const targetPath = args.path as string
    const spinner = ora('Analyzing ruby pinnacle…').start()
    ...
    try {
      ...
      const result = await buildRubyPinnacleResult(files, contents)

      spinner.succeed('Ruby pinnacle analysis complete')
      ...
      spinner.fail('Ruby pinnacle analysis failed')
      throw error
    }
  }
}
