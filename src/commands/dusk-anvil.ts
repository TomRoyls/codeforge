...
} from './dusk-anvil-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './dusk-anvil-format-helpers.js'

/** @example codeforge twilight-forge ./src */
export default class DuskAnvil extends Command {
  static override args = {
    path: Args.string({ default: '.', description: 'Path to analyze' }),
  }

  static override description = 'Twilight forge analysis — transition-grace/dusk-resilience/star-emergence/forge-twilight/ember-wisdom'
  ...
  /** @example DuskAnvil.run() */
  public async run(): Promise<TwilightForgeResult> {
    const { args, flags } = await this.parse(DuskAnvil)
    const targetPath = args.path as string
    const spinner = ora('Analyzing twilight forge…').start()
    ...
    try {
      ...
      const result = await buildTwilightForgeResult(files, contents)

      spinner.succeed('Twilight forge analysis complete')
      ...
      spinner.fail('Twilight forge analysis failed')
      throw error
    }
  }
}
