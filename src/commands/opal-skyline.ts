...
} from './opal-skyline-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './opal-skyline-format-helpers.js'

/** @example codeforge opal-skyline ./src */
export default class OpalSkyline extends Command {
  static override args = {
    path: Args.string({ default: '.', description: 'Path to analyze' }),
  }

  static override description = 'Opal horizon analysis — play-of-color/dawn-clarity/fire-warmth/spectrum-richness/opalescence-quality'
  ...
  /** @example OpalSkyline.run() */
  public async run(): Promise<OpalHorizonResult> {
    const { args, flags } = await this.parse(OpalSkyline)
    const targetPath = args.path as string
    const spinner = ora('Analyzing opal horizon…').start()
    ...
    try {
      ...
      const result = await buildOpalHorizonResult(files, contents)

      spinner.succeed('Opal horizon analysis complete')
      ...
      if (flags.verbose) {
        this.log(chalk.dim(`\nAnalyzed ${files.length} files across ${result.veins.length} veins`))
      }

      return result
    } catch (error: unknown) {
      spinner.fail('Opal horizon analysis failed')
      throw error
    }
  }
}
