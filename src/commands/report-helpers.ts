...
export async function readAnalysisFile(filePath: string): Promise<AnalysisResult> {
  if (!existsSync(filePath)) {
    throw new CLIError(`Analysis file not found: ${filePath}`, {
      code: 'FILE_NOT_FOUND',
      suggestions: [
        'Run "codeforge analyze --format json --output <path>" to generate an analysis file first.',
  ...
  try {
    ...
    throw new CLIError(`Failed to read analysis file: ${filePath}: ${msg}`, {
      code: 'READ_ERROR',
  ...
  try {
    ...
    throw new CLIError(`Invalid JSON in analysis file: ${filePath}`, {
      code: 'E001',
      suggestions: [
  ...
  if (!validateAnalysisResult(parsed)) {
    throw new CLIError(`Invalid JSON in analysis file: ${filePath} — missing required fields (files, summary, timestamp)`, {
      code: 'VALIDATION_ERROR',
      suggestions: ['Regenerate the analysis file using "codeforge analyze --format json".'],
    })
  }
