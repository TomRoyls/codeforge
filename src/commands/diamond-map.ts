...
} from './diamond-map-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './diamond-map-format-helpers.js'

/** @example codeforge diamond-map ./src */
export default class DiamondMap extends Command {
  static override args = {
    path: Args.string({ default: '.', description: 'Path to analyze' }),
  }

  static override description = 'Diamond map analysis — hardness-clarity/cut-precision/map-completeness/fire-dispersion/cartographic-wisdom'
  ...
  /** @example DiamondMap.run() */
  public async run(): Promise<DiamondMapResult> {
    const { args, flags } = await this.parse(DiamondMap)
    const targetPath = args.path as string
    const spinner = ora('Analyzing diamond map…').start()
    ...
    try {
      ...
      const result = await buildDiamondMapResult(files, contents)

      spinner.succeed('Diamond map analysis complete')
      ...
      spinner.fail('Diamond map analysis failed')
      throw error
    }
  }
}
