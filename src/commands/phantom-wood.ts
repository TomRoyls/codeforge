...
import {
  ...
  type PhantomWoodResult,
} from './phantom-wood-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './phantom-wood-format-helpers.js'

/** @example codeforge phantom-wood ./src */
export default class PhantomWood extends Command {
  static override args = {
    path: Args.string({ default: '.', description: 'Path to analyze' }),
  }

  static override description = 'Phantom wood analysis — ethereal-quality/ghost-handling/mist-clarity/root-depth/shadow-wisdom'
  ...
  /** @example PhantomWood.run() */
  public async run(): Promise<PhantomWoodResult> {
    const { args, flags } = await this.parse(PhantomWood)
    const targetPath = args.path as string
    const spinner = ora('Analyzing phantom wood…').start()
    ...
    try {
      ...
      const result = await buildPhantomWoodResult(files, contents)

      spinner.succeed('Phantom wood analysis complete')
      ...
      spinner.fail('Phantom wood analysis failed')
      throw error
    }
  }
}
