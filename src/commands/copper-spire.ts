...
import {
  ...
  gatherFiles,
} from './copper-spire-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './copper-spire-format-helpers.js'

/**
 * @example
 * codeforge copper-spire ./src
 */
export default class CopperSpire extends Command {
  static override description = 'Analyze code patina-wisdom, conductivity-quality, verdigris-beauty, alloy-resilience, and tower-stability'
  ...
  async run(): Promise<void> {
    const { args, flags } = await this.parse(CopperSpire)
    const spinner = ora('Scanning copper spire...').start()
    ...
    try {
      ...
      spinner.text = 'Computing copper spire analysis...'
      const result = await buildCopperSpireResult(
        Array.from(files),
        contents,
        { ext: flags.ext, verbose: flags.verbose },
      )
      spinner.succeed('Copper spire analysis complete')
      ...
      spinner.fail('Copper spire analysis failed')
