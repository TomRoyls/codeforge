...
import {
  ...
  gatherFiles,
} from './jade-meridian-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './jade-meridian-format-helpers.js'

/**
 * @example
 * codeforge jade-meridian ./src
 */
export default class JadeMeridian extends Command {
  static override description = 'Analyze code energy-flow, meridian-balance, chi-vitality, acu-point-precision, and harmonic-resonance'
  ...
  async run(): Promise<void> {
    ...
    const spinner = ora('Scanning jade meridian...').start()
    ...
    try {
      ...
      spinner.text = 'Computing jade meridian analysis...'
      const result = await buildJadeMeridianResult(
        Array.from(files),
        contents,
        { ext: flags.ext, verbose: flags.verbose },
      )
      spinner.succeed('Jade meridian analysis complete')
      ...
      spinner.fail('Jade meridian analysis failed')
