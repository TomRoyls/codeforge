...
import {
  ...
  type StarlightForgeResult,
} from './starlight-foundry-helpers.js'
import {
  formatResultTable,
  formatResultJson,
} from './starlight-foundry-format-helpers.js'

/** @example codeforge starlight-foundry ./src */
export default class StarlightFoundry extends Command {
  static override args = {
    path: Args.string({ default: '.', description: 'Path to analyze' }),
  }

  static override description = 'Starlight forge analysis — celestial-forging/star-hardness/constellation-pattern/nebula-clarity/cosmic-wisdom'
  ...
  /** @example StarlightFoundry.run() */
  public async run(): Promise<StarlightForgeResult> {
    const { args, flags } = await this.parse(StarlightFoundry)
    const targetPath = args.path as string
    const spinner = ora('Analyzing starlight forge…').start()
    ...
    try {
      ...
      const result = await buildStarlightForgeResult(files, contents)

      spinner.succeed('Starlight forge analysis complete')
      ...
      spinner.fail('Starlight forge analysis failed')
      throw error
    }
  }
}
