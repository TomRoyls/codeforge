...
import { formatResultTable, formatResultJson } from './labyrinth-thread-format-helpers.js'

// ─── Command ───────────────────────────────────────────────────────

/**
 * Analyze code maze solvability, path clarity, minotaur resistance, thread strength, and exit accessibility
 * @example
 * codeforge labyrinth-thread ./src
 */
export default class LabyrinthThread extends Command {
  static override description = 'Analyze code maze solvability, path clarity, minotaur resistance, thread strength, and exit accessibility'
  ...
  async run(): Promise<void> {
    ...
    const spinner = ora.default('Unrolling Ariadne\'s thread...').start()
    ...
    try {
      ...
      const result = await buildLabyrinthThreadResult(files, contents, { verbose: flags.verbose })
      spinner.succeed(`Analyzed ${result.stats.totalFiles} file(s) across ${result.stats.totalLevels} level(s)`)
      ...
      if (flags.verbose) {
        this.log(`\nNavigator Grade: ${result.stats.navigatorGrade}`)
        this.log(`Overall Navigability: ${result.stats.overallNavigability}`)
      }
