...
export class ConsoleReporter implements Reporter {
  ...
  private printSourceSnippet(violation: Violation): void {
    ...
    const prefix = this.color ? `${ANSI_CODES.gray}${lineNum} | ${ANSI_CODES.reset}` : `${lineNum} | `;
    ...
    this.printLine(`  ${pointerPrefix}${this.color ? ANSI_CODES.red : ''}${pointer}${this.color ? ANSI_CODES.reset : ''}`);
  }
  ...
  private printSummary(summary: AnalysisResult['summary']): void {
    ...
    if (summary.errorCount > 0) {
      parts.push(this.color ? `${ANSI_CODES.red}✖ ${summary.errorCount} error${summary.errorCount === 1 ? '' : 's'}${ANSI_CODES.reset}` : `✖ ${summary.errorCount} error${summary.errorCount === 1 ? '' : 's'}`);
    }
    
    if (summary.warningCount > 0) {
      parts.push(this.color ? `${ANSI_CODES.yellow}⚠ ${summary.warningCount} warning${summary.warningCount === 1 ? '' : 's'}${ANSI_CODES.reset}` : `⚠ ${summary.warningCount} warning${summary.warningCount === 1 ? '' : 's'}`);
    }
    
    if (summary.infoCount > 0) {
      parts.push(this.color ? `${ANSI_CODES.blue}ℹ ${summary.infoCount} info${ANSI_CODES.reset}` : `ℹ ${summary.infoCount} info`);
    }
    
    if (parts.length === 0) {
      this.printLine(this.color ? `${ANSI_CODES.green}✓ No problems found${ANSI_CODES.reset}` : '✓ No problems found');
    } else {
      this.printLine(parts.join(' '));
    }
    
    this.printLine(this.color ? `${ANSI_CODES.gray}${summary.totalFiles} file${summary.totalFiles === 1 ? '' : 's'} analyzed in ${this.formatTime(summary.totalTime)}${ANSI_CODES.reset}` : `${summary.totalFiles} file${summary.totalFiles === 1 ? '' : 's'} analyzed in ${this.formatTime(summary.totalTime)}`);
  }

  private printVerboseInfo(results: AnalysisResult): void {
    this.printLine('');
    this.printLine(this.color ? `${ANSI_CODES.dim}--- Details ---${ANSI_CODES.reset}` : '--- Details ---');
    
    for (const file of results.files) {
      if (file.violations.length > 0) {
        this.printLine(this.color ? `${ANSI_CODES.dim}${file.filePath}: ${file.violations.length} violation(s) | parse: ${file.stats.parseTime}ms, analysis: ${file.stats.analysisTime}ms${ANSI_CODES.reset}` : `${file.filePath}: ${file.violations.length} violation(s) | parse: ${file.stats.parseTime}ms, analysis: ${file.stats.analysisTime}ms`);
      }
    }
  }

  private reportFull(results: AnalysisResult): void {
    const filesWithViolations = results.files.filter((f: FileAnalysisResult) => f.violations.length > 0);
