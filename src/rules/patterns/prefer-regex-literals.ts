...
import { getArguments, isIdentifier, isLiteral, isNewExpression, toASTNode } from '../../utils/ast-helpers.js'
...
function getPatternInfo(node: unknown): null | { flags: string; pattern: string } {
  if (!isNewExpression(node)) return null

  const args = getArguments(node)
  if (args.length === 0) return { flags: '', pattern: '' }

  const firstArg = args[0]
  if (!isLiteral(firstArg)) return null
  ...
  create(context: RuleContext): RuleVisitor {
    return {
      NewExpression(node: unknown): void {
        ...
        if (!callee || !isIdentifier(callee, 'RegExp')) return
