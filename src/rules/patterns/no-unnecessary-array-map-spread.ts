...
export const noUnnecessaryArrayMapSpreadRule: RuleDefinition = {
    ...
    return {
      CallExpression(node: unknown): void {
        ...
        context.report({
          ...
          message: `arr.map(...items) with spread is unusual. map() expects a callback function.`,
          node: n,
        })
      },
    }
  ...
  meta: {
    docs: {
      category: 'patterns',
      description: 'Warn about arr.map(...items) with spread which is likely a mistake.',
      recommended: false,
