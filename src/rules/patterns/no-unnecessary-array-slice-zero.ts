...
export const noUnnecessaryArraySliceZeroRule: RuleDefinition = {
  create(context: RuleContext): RuleVisitor {
    return {
      CallExpression(node: unknown): void {
        ...
        if (!propNode || propNode.type !== 'Identifier' || propNode.name !== 'slice') return
  ...
  meta: {
    docs: {
      category: 'patterns',
      description:
        'Flag .slice(0) which returns a copy of the entire array. Use spread syntax instead.',
      recommended: false,
      url: 'https://github.com/codeforge-dev/codeforge/blob/main/docs/rules/patterns/no-unnecessary-array-slice-zero.md',
    },
    schema: [],
    severity: 'warn',
    type: 'suggestion',
...
export default noUnnecessaryArraySliceZeroRule
