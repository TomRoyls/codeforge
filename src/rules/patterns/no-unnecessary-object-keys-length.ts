  ...
  create(context: RuleContext): RuleVisitor {
    return {
      CallExpression(node: unknown): void {
        ...
        if (outerCallee.type !== 'MemberExpression' || outerCallee.computed) return
        if (outerCallee.property.type !== 'Identifier' || outerCallee.property.name !== 'length') return
        const inner = outerCallee.object
        if (inner.type !== 'CallExpression') return
        if (inner.arguments.length !== 1) return
        const innerCallee = inner.callee
        if (innerCallee.type !== 'MemberExpression' || innerCallee.computed) return
        if (innerCallee.object.type !== 'Identifier' || innerCallee.object.name !== 'Object') return
        if (innerCallee.property.type !== 'Identifier' || innerCallee.property.name !== 'keys') return
        context.report({
