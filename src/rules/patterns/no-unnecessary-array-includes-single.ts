...
export const noUnnecessaryArrayIncludesSingleRule: RuleDefinition = {
  create(context: RuleContext): RuleVisitor {
    return {
      CallExpression(node: unknown): void {
        ...
        if (propNode.name !== 'includes') return

        const obj = c.object
        if (!obj || typeof obj !== 'object') return

        const objNode = toASTNode(obj) as Record<string, unknown>
        if (!objNode || objNode.type !== 'ArrayExpression') return

        const elements = objNode.elements
  ...
  meta: {
    docs: {
      category: 'patterns',
      description:
        'Disallow .includes() on single-element array literals. Use direct equality comparison instead.',
      recommended: false,
      url: 'https://github.com/codeforge-dev/codeforge/blob/main/docs/rules/patterns/no-unnecessary-array-includes-single.md',
    },
    schema: [],
    severity: 'warn',
    type: 'suggestion',
...
export default noUnnecessaryArrayIncludesSingleRule
