...
function checkAsyncForEachCallback(
  node: Node,
...
export const noMisusedPromisesRule: RuleDefinition<NoMisusedPromisesOptions> = {
  meta: {
    ...
    description:
      "Disallow promises in fire-and-forget contexts like forEach with async callbacks",
...
export function analyzeMisusedPromises(
  node: Node,
  context: VisitorContext,
): RuleViolation[] {
  const violation = checkAsyncForEachCallback(node, context);
