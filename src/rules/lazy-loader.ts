  ...
  for (const ruleId of patternRules) {
    ...
    loaders[ruleId] = () =>
      patternsModule().then((m) => {
        // Try with "Rule" suffix first, then without, then fallback to searching by key
        const rule = (m[exportName] ?? m[camelCaseName as keyof typeof m]) as PluginRuleDefinition | undefined
        if (rule) {
          return { [ruleId]: adaptPluginRule(rule, ruleId) }
        }

        // Fallback: search for a key that matches after normalizing casing
        // This handles cases like "NaN" vs "Nan" in export names
        for (const [key, value] of Object.entries(m)) {
          if (key.toLowerCase() === camelCaseName.toLowerCase() || key.toLowerCase() === exportName.toLowerCase()) {
            return { [ruleId]: adaptPluginRule(value as PluginRuleDefinition, ruleId) }
          }
        }

        throw new SystemError(
          `Rule ${ruleId} not found in patterns module (tried exports: ${exportName}, ${camelCaseName}, case-insensitive fallback)`,
          { code: 'E500' },
        )
      })
  }
