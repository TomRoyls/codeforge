...
export class Reporter {
  ...
  private formatMarkdown(report: AnalysisReport): string {
    ...
    lines.push(`| Files with Violations | ${report.files.filter(f => f.violations.length > 0).length} |`);
    ...
    if (filesWithViolations.length > 0) {
      ...
      for (const file of filesWithViolations) {
        ...
        for (const v of file.violations) {
          ...
          lines.push(`${icon} **${v.ruleId}** at line ${v.range.start.line}:${v.range.start.column} - ${v.message}`);
        }
        lines.push('');
      }
    } else {
    ...
    for (const file of report.files) {
      for (const v of file.violations) {
        results.push({
          ...
          severity: v.severity === 'error' ? 'critical' : v.severity === 'warning' ? 'major' : 'minor',
          location: {
            path: file.filePath,
            lines: {
