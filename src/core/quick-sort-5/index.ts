export class QuickSort5 {
  ...
  sort(arr: number[]): number[] {
    ...
    this.introSort(copy, 0, copy.length - 1, 2 * Math.floor(Math.log2(copy.length)));
    return copy;
  }
  ...
  partialSort(arr: number[], k: number): number[] {
    if (arr.length <= k) {
      return this.sort(arr);
    }
    const copy = arr.slice();
    this.partialQuickSort(copy, 0, copy.length - 1, k);
    return copy.slice(0, k);
  }

  private partialQuickSort(arr: number[], low: number, high: number, k: number): void {
    if (low < high && low < k) {
      const pivotIndex = this.partition(arr, low, high);
      this.partialQuickSort(arr, low, pivotIndex - 1, k);
      this.partialQuickSort(arr, pivotIndex + 1, high, k);
    }
  }

  selectKth(arr: number[], k: number): number | undefined {
    if (k < 1 || k > arr.length) {
      return undefined;
    }
    const copy = arr.slice();
    return this.selectKthHelper(copy, 0, copy.length - 1, k - 1);
  }

  private selectKthHelper(arr: number[], left: number, right: number, k: number): number {
    while (left < right) {
  ...
  getTimeComplexity(): string {
    return 'O(n log n) average, O(n²) worst, O(n) space';
  }
