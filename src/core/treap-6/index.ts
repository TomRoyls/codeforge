...
export class Treap6<T> {
  ...
  split(value: T): [Treap6<T>, Treap6<T>] {
    const [leftRoot, rightRoot] = this.splitNode(this.root, value)

    const leftTreap = new Treap6<T>(this.comparator)
    leftTreap.root = leftRoot
    leftTreap._size = this.countNodes(leftRoot)

    const rightTreap = new Treap6<T>(this.comparator)
  ...
  merge(otherTree: Treap6<T>): this {
    this.root = this.mergeNodes(this.root, otherTree.root)
  ...
  private rangeSearchNode(node: TreapNode<T> | null, low: T, high: T, result: T[]): void {
    if (!node) {
      return
    }
    ...
    if (cmpLow < 0) {
      this.rangeSearchNode(node.left, low, high, result)
    }
    ...
    if (cmpHigh > 0) {
      this.rangeSearchNode(node.right, low, high, result)
    }
  }

  rangeSearch(low: T, high: T): T[] {
    const result: T[] = []
    this.rangeSearchNode(this.root, low, high, result)
    return result
  }
  ...
  private rankNode(node: TreapNode<T> | null, value: T): number {
    ...
    if (cmp < 0) {
      ...
      return 1 + this.countNodes(node.left) + this.rankNode(node.right, value)
    } else {
  ...
  private selectNode(node: TreapNode<T> | null, k: number): T | undefined {
    if (!node) {
      return undefined
    }
