export class FibonacciSearch {
  ...
  get name(): string { return 'FibonacciSearch' }
  toString(): string { return JSON.stringify({ name: this.name }) }
  toJSON(): Record<string, string> { return { name: this.name } }
  clone(): FibonacciSearch { return new FibonacciSearch() }
  equals(other: unknown): boolean { return other instanceof FibonacciSearch }
  toArray(): string[] { return ['fibonacci-search'] }
}
