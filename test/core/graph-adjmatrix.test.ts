import { describe, it, expect } from 'vitest';
import { GraphAdjMatrix } from '../../src/core/graph-adjmatrix/index.js';

// ─── Constructor ───

describe('GraphAdjMatrix constructor', () => {
  it('creates undirected unweighted graph by default', () => {
    const g = new GraphAdjMatrix<number>();
    expect(g.isEmpty()).toBe(true);
  });

  it('creates directed graph', () => {
    const g = new GraphAdjMatrix<number>(true);
    expect(g.isEmpty()).toBe(true);
  });

  it('creates weighted graph', () => {
    const g = new GraphAdjMatrix<number>(false, true);
    expect(g.isEmpty()).toBe(true);
  });

  it('creates directed weighted graph', () => {
    const g = new GraphAdjMatrix<number>(true, true);
    expect(g.isEmpty()).toBe(true);
  });
});

// ─── addVertex() ───

describe('GraphAdjMatrix.addVertex', () => {
  it('adds a vertex', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    expect(g.hasVertex(1)).toBe(true);
    expect(g.vertexCount).toBe(1);
  });

  it('does not duplicate vertices', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    g.addVertex(1);
    expect(g.vertexCount).toBe(1);
  });

  it('adds multiple vertices', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    g.addVertex(2);
    g.addVertex(3);
    expect(g.vertexCount).toBe(3);
  });

  it('works with string vertices', () => {
    const g = new GraphAdjMatrix<string>();
    g.addVertex('a');
    g.addVertex('b');
    expect(g.hasVertex('a')).toBe(true);
    expect(g.hasVertex('b')).toBe(true);
  });
});

// ─── removeVertex() ───

describe('GraphAdjMatrix.removeVertex', () => {
  it('removes a vertex', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    g.addVertex(2);
    g.removeVertex(1);
    expect(g.hasVertex(1)).toBe(false);
    expect(g.vertexCount).toBe(1);
  });

  it('does nothing for non-existent vertex', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    g.removeVertex(99);
    expect(g.vertexCount).toBe(1);
  });

  it('removes associated edges', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    g.addVertex(2);
    g.addVertex(3);
    g.addEdge(1, 2);
    g.addEdge(1, 3);
    g.removeVertex(1);
    expect(g.hasEdge(2, 1)).toBe(false);
    expect(g.hasEdge(3, 1)).toBe(false);
    expect(g.hasVertex(2)).toBe(true);
    expect(g.hasVertex(3)).toBe(true);
  });
});

// ─── addEdge() ───

describe('GraphAdjMatrix.addEdge', () => {
  it('adds an edge in undirected graph', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    g.addVertex(2);
    g.addEdge(1, 2);
    expect(g.hasEdge(1, 2)).toBe(true);
    expect(g.hasEdge(2, 1)).toBe(true);
  });

  it('adds a directed edge', () => {
    const g = new GraphAdjMatrix<number>(true);
    g.addVertex(1);
    g.addVertex(2);
    g.addEdge(1, 2);
    expect(g.hasEdge(1, 2)).toBe(true);
    expect(g.hasEdge(2, 1)).toBe(false);
  });

  it('adds weighted edge', () => {
    const g = new GraphAdjMatrix<number>(false, true);
    g.addVertex(1);
    g.addVertex(2);
    g.addEdge(1, 2, 5);
    expect(g.getEdgeWeight(1, 2)).toBe(5);
    expect(g.getEdgeWeight(2, 1)).toBe(5);
  });

  it('does nothing when vertices do not exist', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    g.addEdge(1, 2);
    expect(g.hasEdge(1, 2)).toBe(false);
  });
});

// ─── removeEdge() ───

describe('GraphAdjMatrix.removeEdge', () => {
  it('removes an edge in undirected graph', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    g.addVertex(2);
    g.addEdge(1, 2);
    g.removeEdge(1, 2);
    expect(g.hasEdge(1, 2)).toBe(false);
    expect(g.hasEdge(2, 1)).toBe(false);
  });

  it('removes directed edge in one direction only', () => {
    const g = new GraphAdjMatrix<number>(true);
    g.addVertex(1);
    g.addVertex(2);
    g.addEdge(1, 2);
    g.addEdge(2, 1);
    g.removeEdge(1, 2);
    expect(g.hasEdge(1, 2)).toBe(false);
    expect(g.hasEdge(2, 1)).toBe(true);
  });
});

// ─── hasVertex() / hasEdge() ───

describe('GraphAdjMatrix.hasVertex / hasEdge', () => {
  it('hasVertex returns false for missing vertex', () => {
    const g = new GraphAdjMatrix<number>();
    expect(g.hasVertex(1)).toBe(false);
  });

  it('hasEdge returns false for non-existent vertices', () => {
    const g = new GraphAdjMatrix<number>();
    expect(g.hasEdge(1, 2)).toBe(false);
  });
});

// ─── getEdgeWeight() ───

describe('GraphAdjMatrix.getEdgeWeight', () => {
  it('returns weight in weighted graph', () => {
    const g = new GraphAdjMatrix<number>(false, true);
    g.addVertex(1);
    g.addVertex(2);
    g.addEdge(1, 2, 10);
    expect(g.getEdgeWeight(1, 2)).toBe(10);
  });

  it('returns null for non-existent edge', () => {
    const g = new GraphAdjMatrix<number>(false, true);
    g.addVertex(1);
    g.addVertex(2);
    expect(g.getEdgeWeight(1, 2)).toBeNull();
  });

  it('returns null in unweighted graph', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    g.addVertex(2);
    g.addEdge(1, 2);
    expect(g.getEdgeWeight(1, 2)).toBeNull();
  });

  it('returns null for non-existent vertices', () => {
    const g = new GraphAdjMatrix<number>(false, true);
    expect(g.getEdgeWeight(1, 2)).toBeNull();
  });
});

// ─── getNeighbors() ───

describe('GraphAdjMatrix.getNeighbors', () => {
  it('returns neighbors of a vertex', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    g.addVertex(2);
    g.addVertex(3);
    g.addEdge(1, 2);
    g.addEdge(1, 3);
    const neighbors = g.getNeighbors(1);
    expect(neighbors).toContain(2);
    expect(neighbors).toContain(3);
    expect(neighbors.length).toBe(2);
  });

  it('returns empty array for isolated vertex', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    expect(g.getNeighbors(1)).toEqual([]);
  });

  it('returns empty array for non-existent vertex', () => {
    const g = new GraphAdjMatrix<number>();
    expect(g.getNeighbors(99)).toEqual([]);
  });
});

// ─── getVertices() / getEdges() ───

describe('GraphAdjMatrix.getVertices / getEdges', () => {
  it('getVertices returns all vertices', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    g.addVertex(2);
    g.addVertex(3);
    const verts = g.getVertices();
    expect(verts.length).toBe(3);
    expect(verts).toContain(1);
    expect(verts).toContain(2);
    expect(verts).toContain(3);
  });

  it('getEdges returns all edges in undirected graph', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    g.addVertex(2);
    g.addVertex(3);
    g.addEdge(1, 2);
    g.addEdge(2, 3);
    const edges = g.getEdges();
    expect(edges.length).toBe(2);
  });

  it('getEdges returns all edges in directed graph', () => {
    const g = new GraphAdjMatrix<number>(true);
    g.addVertex(1);
    g.addVertex(2);
    g.addEdge(1, 2);
    g.addEdge(2, 1);
    const edges = g.getEdges();
    expect(edges.length).toBe(2);
  });
});

// ─── vertexCount / edgeCount ───

describe('GraphAdjMatrix.vertexCount / edgeCount', () => {
  it('tracks vertex count', () => {
    const g = new GraphAdjMatrix<number>();
    expect(g.vertexCount).toBe(0);
    g.addVertex(1);
    expect(g.vertexCount).toBe(1);
    g.addVertex(2);
    expect(g.vertexCount).toBe(2);
  });

  it('tracks edge count in undirected graph', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    g.addVertex(2);
    g.addVertex(3);
    g.addEdge(1, 2);
    g.addEdge(2, 3);
    expect(g.edgeCount).toBe(2);
  });

  it('tracks edge count in directed graph', () => {
    const g = new GraphAdjMatrix<number>(true);
    g.addVertex(1);
    g.addVertex(2);
    g.addEdge(1, 2);
    g.addEdge(2, 1);
    expect(g.edgeCount).toBe(2);
  });
});

// ─── isEmpty() / clear() ───

describe('GraphAdjMatrix.isEmpty / clear', () => {
  it('isEmpty returns true for empty graph', () => {
    const g = new GraphAdjMatrix<number>();
    expect(g.isEmpty()).toBe(true);
  });

  it('isEmpty returns false after adding vertices', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    expect(g.isEmpty()).toBe(false);
  });

  it('clear removes all vertices and edges', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    g.addVertex(2);
    g.addEdge(1, 2);
    g.clear();
    expect(g.isEmpty()).toBe(true);
    expect(g.vertexCount).toBe(0);
    expect(g.edgeCount).toBe(0);
  });
});

// ─── bfs() ───

describe('GraphAdjMatrix.bfs', () => {
  it('traverses vertices in BFS order', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    g.addVertex(2);
    g.addVertex(3);
    g.addVertex(4);
    g.addEdge(1, 2);
    g.addEdge(1, 3);
    g.addEdge(2, 4);
    const visited: number[] = [];
    g.bfs(1, (v) => visited.push(v));
    expect(visited[0]).toBe(1);
    expect(visited).toContain(2);
    expect(visited).toContain(3);
    expect(visited).toContain(4);
    expect(visited.length).toBe(4);
  });

  it('does nothing for non-existent start vertex', () => {
    const g = new GraphAdjMatrix<number>();
    const visited: number[] = [];
    g.bfs(99, (v) => visited.push(v));
    expect(visited).toEqual([]);
  });

  it('handles single vertex', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    const visited: number[] = [];
    g.bfs(1, (v) => visited.push(v));
    expect(visited).toEqual([1]);
  });
});

// ─── dfs() ───

describe('GraphAdjMatrix.dfs', () => {
  it('traverses vertices', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    g.addVertex(2);
    g.addVertex(3);
    g.addEdge(1, 2);
    g.addEdge(1, 3);
    const visited: number[] = [];
    g.dfs(1, (v) => visited.push(v));
    expect(visited[0]).toBe(1);
    expect(visited.length).toBe(3);
  });

  it('does nothing for non-existent start vertex', () => {
    const g = new GraphAdjMatrix<number>();
    const visited: number[] = [];
    g.dfs(99, (v) => visited.push(v));
    expect(visited).toEqual([]);
  });
});

// ─── degree() ───

describe('GraphAdjMatrix.degree', () => {
  it('returns degree of a vertex', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    g.addVertex(2);
    g.addVertex(3);
    g.addEdge(1, 2);
    g.addEdge(1, 3);
    expect(g.degree(1)).toBe(2);
  });

  it('returns 0 for isolated vertex', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    expect(g.degree(1)).toBe(0);
  });

  it('returns 0 for non-existent vertex', () => {
    const g = new GraphAdjMatrix<number>();
    expect(g.degree(99)).toBe(0);
  });

  it('returns out-degree in directed graph', () => {
    const g = new GraphAdjMatrix<number>(true);
    g.addVertex(1);
    g.addVertex(2);
    g.addVertex(3);
    g.addEdge(1, 2);
    g.addEdge(1, 3);
    g.addEdge(2, 1);
    expect(g.degree(1)).toBe(2);
    expect(g.degree(2)).toBe(1);
  });
});

// ─── hasPath() ───

describe('GraphAdjMatrix.hasPath', () => {
  it('returns true for connected vertices', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    g.addVertex(2);
    g.addVertex(3);
    g.addEdge(1, 2);
    g.addEdge(2, 3);
    expect(g.hasPath(1, 3)).toBe(true);
  });

  it('returns false for disconnected vertices', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    g.addVertex(2);
    g.addVertex(3);
    g.addEdge(1, 2);
    expect(g.hasPath(1, 3)).toBe(false);
  });

  it('returns true for same vertex', () => {
    const g = new GraphAdjMatrix<number>();
    g.addVertex(1);
    expect(g.hasPath(1, 1)).toBe(true);
  });

  it('returns false for non-existent vertices', () => {
    const g = new GraphAdjMatrix<number>();
    expect(g.hasPath(1, 2)).toBe(false);
  });

  it('returns true for multi-hop directed path', () => {
    const g = new GraphAdjMatrix<number>(true);
    g.addVertex(1);
    g.addVertex(2);
    g.addVertex(3);
    g.addEdge(1, 2);
    g.addEdge(2, 3);
    expect(g.hasPath(1, 3)).toBe(true);
    expect(g.hasPath(3, 1)).toBe(false);
  });
});
