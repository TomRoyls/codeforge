...
describe('FibonacciSet', () => {
  ...
  describe('add', () => {
    ...
    it('adds in reverse order', () => {
      const s = new FibonacciSet<number>()
      for (let i = 100; i >= 1; i--) s.add(i)
      expect(s.size).toBe(100)
      expect(s.toArray()).toEqual(Array.from({ length: 100 }, (_, i) => i + 1))
    })

    it('adds in sorted order', () => {
      const s = new FibonacciSet<number>()
  ...
  describe('delete', () => {
    ...
    it('deletes shuffled', () => {
      ...
      for (const n of nums) s.add(n)
      const toDelete = shuffle(nums).slice(0, 10)
      for (const n of toDelete) s.delete(n)
    ...
    it('maintains correctness after many deletions', () => {
      ...
      for (let i = 1; i <= 25; i++) s.delete(i)
      expect(s.toArray()).toEqual(Array.from({ length: 25 }, (_, i) => i + 26))
    })
  })
  ...
  describe('has', () => {
    ...
    it('returns correct results for many elements', () => {
      const s = new FibonacciSet<number>()
