...
import { IndexedSet } from '../../src/core/indexed-set/indexed-set.js'

function assertSorted<T>(s: IndexedSet<T>): void {
  const arr = s.toArray()
...
describe('IndexedSet', () => {
  describe('construction', () => {
    it('should create empty set with no arguments', () => {
      const s = new IndexedSet<number>()
      expect(s.size).toBe(0)
      expect(s.isEmpty()).toBe(true)
    })

    it('should accept custom comparator function', () => {
      const s = new IndexedSet<number>({ comparator: (a, b) => b - a })
      s.add(1)
      s.add(2)
      s.add(3)
    ...
    it('should accept string comparator', () => {
      const s = new IndexedSet<string>({ comparator: (a, b) => a.localeCompare(b) })
      s.add('banana')
      s.add('apple')
      s.add('cherry')
      expect(s.toArray()).toEqual(['apple', 'banana', 'cherry'])
  ...
  describe('add', () => {
    ...
    it('should return true when adding new value', () => {
      const s = new IndexedSet<number>()
  ...
  describe('atIndex', () => {
    ...
    it('should return elements in sorted order via iteration', () => {
      const s = new IndexedSet<number>()
      s.add(5)
      s.add(1)
      s.add(3)
  ...
  describe('indexOf', () => {
    ...
    it('should return correct index for large set', () => {
      const s = new IndexedSet<number>()
      for (let i = 0; i < 100; i++) s.add(i)
  ...
  describe('rank', () => {
    it('should return number of elements less than value', () => {
      const s = new IndexedSet<number>()
      s.add(1)
      s.add(3)
      s.add(5)
      expect(s.rank(1)).toBe(0)
      expect(s.rank(3)).toBe(1)
      expect(s.rank(5)).toBe(2)
    })

    it('should count elements less than non-existent value', () => {
      const s = new IndexedSet<number>()
      s.add(1)
      s.add(3)
      s.add(5)
      expect(s.rank(2)).toBe(1)
      expect(s.rank(4)).toBe(2)
    })

    it('should return 0 for value less than min', () => {
      const s = new IndexedSet<number>()
      s.add(5)
      s.add(10)
      expect(s.rank(1)).toBe(0)
    ...
    it('should return size for value greater than max', () => {
      const s = new IndexedSet<number>()
      s.add(5)
      s.add(10)
      expect(s.rank(20)).toBe(2)
    })
    ...
    it('should match indexOf for existing values', () => {
      const s = new IndexedSet<number>()
      for (let i = 0; i < 50; i++) s.add(i * 2)
      for (let i = 0; i < 50; i++) {
        expect(s.rank(i * 2)).toBe(s.indexOf(i * 2))
      }
    })
  })
  ...
  describe('from factory', () => {
    it('should create set from iterable', () => {
      const s = IndexedSet.from([3, 1, 2])
      expect(s.size).toBe(3)
      expect(s.toArray()).toEqual([1, 2, 3])
    })

    it('should handle empty iterable', () => {
      const s = IndexedSet.from<number>([])
      expect(s.isEmpty()).toBe(true)
    })

    it('should accept custom comparator', () => {
      const s = IndexedSet.from([1, 2, 3], { comparator: (a, b) => b - a })
      expect(s.toArray()).toEqual([3, 2, 1])
    })

    it('should handle duplicate values in iterable', () => {
      const s = IndexedSet.from([1, 1, 1])
      expect(s.size).toBe(1)
    })
    ...
    it('should work with Set as input', () => {
      const s = IndexedSet.from(new Set([3, 1, 2]))
      expect(s.toArray()).toEqual([1, 2, 3])
    })
  })
  ...
  describe('set operations', () => {
    it('union should combine two sets', () => {
      const a = new IndexedSet<number>()
      a.add(1)
      a.add(2)
      const b = new IndexedSet<number>()
      b.add(2)
      b.add(3)
      const u = a.union(b)
  ...
  describe('large sets', () => {
    ...
    it('should handle atIndex on large set', () => {
      ...
      expect(s.atIndex(5000)).toBe(5000)
  ...
  describe('stats', () => {
    ...
    it('should return correct stats for large set', () => {
      ...
      expect(stats.maxValue).toBe(999)
  ...
  describe('index bounds checking', () => {
    it('should return undefined for atIndex(-1)', () => {
      const s = new IndexedSet<number>()
      s.add(1)
      expect(s.atIndex(-1)).toBeUndefined()
    })

    it('should return undefined for atIndex beyond size', () => {
      const s = new IndexedSet<number>()
      s.add(1)
      s.add(2)
    ...
    it('should return undefined for atIndex(0) on empty set', () => {
      const s = new IndexedSet<number>()
      expect(s.atIndex(0)).toBeUndefined()
    })

    it('should maintain correct indices after many operations', () => {
      const s = new IndexedSet<number>()
      for (let i = 0; i < 100; i++) s.add(i)
      s.delete(25)
      s.delete(50)
