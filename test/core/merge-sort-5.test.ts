...
describe('MergeSort5', () => {
  ...
  describe('sortRange', () => {
    it('sorts a sub-range of the array', () => {
      const sorter = new MergeSort5()
      expect(sorter.sortRange([5, 3, 1, 4, 2], 1, 4)).toEqual([1, 3, 4])
    })

    it('returns empty for invalid range (start >= end)', () => {
      const sorter = new MergeSort5()
      expect(sorter.sortRange([1, 2, 3], 2, 2)).toEqual([])
    })

    it('sorts single element range', () => {
      const sorter = new MergeSort5()
      expect(sorter.sortRange([5, 3, 1], 1, 2)).toEqual([3])
    })

    it('sorts full range', () => {
      const sorter = new MergeSort5()
      expect(sorter.sortRange([3, 1, 2], 0, 3)).toEqual([1, 2, 3])
    })
  })
  ...
  describe('merge', () => {
    ...
    it('uses custom comparator', () => {
      const sorter = new MergeSort5((a, b) => b - a)
      expect(sorter.merge([5, 3], [4, 1])).toEqual([5, 4, 3, 1])
    })
  })
  ...
  describe('edge cases', () => {
    ...
    it('large in-place sort', () => {
      const sorter = new MergeSort5()
      const arr = Array.from({ length: 64 }, (_, i) => 64 - i)
      sorter.sortInPlace(arr)
      expect(arr).toEqual(Array.from({ length: 64 }, (_, i) => i + 1))
    })
  })
})
