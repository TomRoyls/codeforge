...
describe("GoldenRatioSearch", () => {
  ...
  it("handles getIterations after findMinimum", () => {
    const fn = (x: number) => (x - 3) * (x - 3);
    const search = new GoldenRatioSearch(fn);
    search.findMinimum(0, 10);
    expect(search.getIterations()).toBeGreaterThan(0);
  ...
  it("handles findMaximum", () => {
    ...
    it("handles findMinimum on narrow range", () => {
      ...
      expect(result.x).toBeGreaterThanOrEqual(2.9);
      expect(result.x).toBeLessThanOrEqual(3.1);
    });
   it("handles findMinimum on linear function", () => {
     const fn = (x: number) => x;
     const search = new GoldenRatioSearch(fn);
     const result = search.findMinimum(0, 10);
