...
describe('JUnitReporter', () => {
  ...
  describe('report to console', () => {
    ...
    test('should include summary counts in testsuites', () => {
      ...
      const results = createMockAnalysisResult({
        files: [],
        summary: { totalFiles: 5, filesWithViolations: 2, errorCount: 3, warningCount: 2, infoCount: 1, totalTime: 500 },
      });
      reporter.report(results);
      const output = consoleSpy.mock.calls[0][0] as string;
    ...
    test('should include time attribute', () => {
      ...
      const results = createMockAnalysisResult({
        files: [],
        summary: { totalFiles: 0, filesWithViolations: 0, errorCount: 0, warningCount: 0, infoCount: 0, totalTime: 1500 },
      });
      reporter.report(results);
      const output = consoleSpy.mock.calls[0][0] as string;
    ...
    test('should create testsuite for each file', () => {
      ...
      const results = createMockAnalysisResult({
        files: [
          createMockFileResult('file1.ts', []),
          createMockFileResult('file2.ts', []),
        ],
        summary: { totalFiles: 2, filesWithViolations: 0, errorCount: 0, warningCount: 0, infoCount: 0, totalTime: 20 },
      });
      reporter.report(results);
      const output = consoleSpy.mock.calls[0][0] as string;
  ...
  describe('testcase generation', () => {
    test('should create testcase for error severity', () => {
      const reporter = new JUnitReporter();
      const results = createMockAnalysisResult({
        files: [createMockFileResult('test.ts', [createMockViolation({ severity: 'error', message: 'Error message' })])],
        summary: { totalFiles: 1, filesWithViolations: 1, errorCount: 1, warningCount: 0, infoCount: 0, totalTime: 30 },
      });
      reporter.report(results);
      const output = consoleSpy.mock.calls[0][0] as string;
    ...
    test('should create testcase with failure for warning severity', () => {
      const reporter = new JUnitReporter();
      const results = createMockAnalysisResult({
        files: [createMockFileResult('test.ts', [createMockViolation({ severity: 'warning', message: 'Warning message' })])],
        summary: { totalFiles: 1, filesWithViolations: 1, errorCount: 0, warningCount: 1, infoCount: 0, totalTime: 30 },
      });
      reporter.report(results);
      const output = consoleSpy.mock.calls[0][0] as string;
      expect(output).toContain('<testcase');
    ...
    test('should create testcase with failure for info severity', () => {
      const reporter = new JUnitReporter();
      const results = createMockAnalysisResult({
        files: [createMockFileResult('test.ts', [createMockViolation({ severity: 'info', message: 'Info message' })])],
        summary: { totalFiles: 1, filesWithViolations: 1, errorCount: 0, warningCount: 0, infoCount: 1, totalTime: 30 },
      });
      reporter.report(results);
      const output = consoleSpy.mock.calls[0][0] as string;
      expect(output).toContain('<testcase');
    ...
    test('should include location in error element', () => {
      const reporter = new JUnitReporter();
      const results = createMockAnalysisResult({
        files: [createMockFileResult('src/app.ts', [createMockViolation({ severity: 'error', line: 42, column: 10 })])],
        summary: { totalFiles: 1, filesWithViolations: 1, errorCount: 1, warningCount: 0, infoCount: 0, totalTime: 30 },
      });
      reporter.report(results);
      const output = consoleSpy.mock.calls[0][0] as string;
    ...
    test('should include suggestion when provided', () => {
      const reporter = new JUnitReporter();
      const results = createMockAnalysisResult({
        files: [createMockFileResult('test.ts', [createMockViolation({ suggestion: 'Use const instead' })])],
        summary: { totalFiles: 1, filesWithViolations: 1, errorCount: 1, warningCount: 0, infoCount: 0, totalTime: 30 },
      });
      reporter.report(results);
      const output = consoleSpy.mock.calls[0][0] as string;
    ...
    test('should include ruleId in testcase name', () => {
      const reporter = new JUnitReporter();
      const results = createMockAnalysisResult({
        files: [createMockFileResult('test.ts', [createMockViolation({ ruleId: 'no-unused-vars' })])],
        summary: { totalFiles: 1, filesWithViolations: 1, errorCount: 1, warningCount: 0, infoCount: 0, totalTime: 30 },
      });
      reporter.report(results);
      const output = consoleSpy.mock.calls[0][0] as string;
  ...
  describe('edge cases', () => {
    ...
    test('should handle deeply nested file path', () => {
      const reporter = new JUnitReporter({ outputPath: './reports/2024/01/15/deep/nested/junit.xml' });
      const results = createMockAnalysisResult({
        files: [],
        summary: { totalFiles: 0, filesWithViolations: 0, errorCount: 0, warningCount: 0, infoCount: 0, totalTime: 0 },
      });
    ...
    test('should handle violation without optional fields', () => {
      ...
      const results = createMockAnalysisResult({
        ...
        summary: { totalFiles: 1, filesWithViolations: 1, errorCount: 1, warningCount: 0, infoCount: 0, totalTime: 10 },
      });
      reporter.report(results);
      const output = consoleSpy.mock.calls[0][0] as string;
    ...
    test('should handle large message with newlines and tabs', () => {
      const reporter = new JUnitReporter();
      const results = createMockAnalysisResult({
        files: [createMockFileResult('test.ts', [createMockViolation({ message: 'Line 1\nLine 2\tIndented' })])],
        summary: { totalFiles: 1, filesWithViolations: 1, errorCount: 1, warningCount: 0, infoCount: 0, totalTime: 30 },
      });
      reporter.report(results);
      const output = consoleSpy.mock.calls[0][0] as string;
