...
describe('Reporter', () => {
  ...
  describe('formatJson', () => {
    test('serializes results with proper formatting', () => {
      ...
      const report = createReport({
        ...
        summary: { totalFiles: 1, totalViolations: 1, errors: 1, warnings: 0, info: 0, duration: 123 },
      });
      const output = reporter.formatReport(report);
      const parsed = JSON.parse(output);
    ...
    test('handles multiple files with violations', () => {
      ...
      const report = createReport({
        ...
        summary: { totalFiles: 3, totalViolations: 2, errors: 2, warnings: 0, info: 0, duration: 100 },
      });
      const output = reporter.formatReport(report);
      const parsed = JSON.parse(output);
  ...
  describe('formatConsole', () => {
    ...
    test('formats duration with 2 decimal places', () => {
      const reporter = new Reporter({ ...defaultOptions, format: 'console' });
      const report = createReport({
        summary: { totalFiles: 0, totalViolations: 0, errors: 0, warnings: 0, info: 0, duration: 1234.567 },
      });
      const output = reporter.formatReport(report);
    ...
    test('pads severity labels to 7 characters', () => {
      ...
      const report = createReport({
        ...
        summary: { totalFiles: 3, totalViolations: 3, errors: 1, warnings: 1, info: 1, duration: 50 },
      });
      const output = reporter.formatReport(report);
