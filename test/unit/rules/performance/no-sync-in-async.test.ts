...
function createMockAsyncFunctionWithCalls(
  ...
  (funcNode as unknown as { getDescendantsOfKind: (kind: number) => Node[] }).getDescendantsOfKind = vi.fn(
    (kind: number) => {
...
function createMockSyncFunctionWithCalls(
  callExpressions: Node[] = [],
): FunctionLikeNode {
  const funcNode = createMockNode({
    kind: SyntaxKind.FunctionDeclaration,
...
describe("sync operation detection", () => {
  test("detects readFileSync in async function", () => {
    const callNode = createMockCallExpression({ text: "fs.readFileSync('path')", start: 10, end: 35 });
  ...
  test("detects multiple sync operations", () => {
    ...
    const callNode2 = createMockCallExpression({ text: "fs.writeFileSync('b')", start: 40, end: 60 });
