...
describe('no-unnecessary-encode-uri-component-spread', () => {

  describe('reports on encodeURIComponent with single spread argument', () => {
    test('should report encodeURIComponent(...items)', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('encodeURIComponent', [makeSpreadArg({ type: 'Identifier', name: 'items' })], 1, 0, 1, 20)
      noUnnecessaryEncodeUriComponentSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toContain('encodeURIComponent')
    })

    test('should report encodeURIComponent(...arr)', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('encodeURIComponent', [makeSpreadArg({ type: 'Identifier', name: 'arr' })], 1, 0, 1, 20)
      noUnnecessaryEncodeUriComponentSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
    })

    test('should report encodeURIComponent(...[1, 2, 3])', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('encodeURIComponent', [makeSpreadArg({ type: 'ArrayExpression', elements: [] })], 1, 0, 1, 20)
      noUnnecessaryEncodeUriComponentSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
    })
  })
  ...
  describe('does not report when there is no spread', () => {
    test('should not report encodeURIComponent(x)', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('encodeURIComponent', [{ type: 'Identifier', name: 'x' }], 1, 0, 1, 10)
      noUnnecessaryEncodeUriComponentSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(0)
    })

    test('should not report encodeURIComponent("hello")', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('encodeURIComponent', [{ type: 'Literal', value: 'hello' }], 1, 0, 1, 10)
      noUnnecessaryEncodeUriComponentSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(0)
    })
    ...
    test('should not report encodeURIComponent()', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('encodeURIComponent', [], 1, 0, 1, 10)
      noUnnecessaryEncodeUriComponentSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(0)
    })
  })
  ...
  describe('does not report with multiple arguments', () => {
    test('should not report encodeURIComponent(...items, extra)', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('encodeURIComponent', [makeSpreadArg({ type: 'Identifier', name: 'items' }), { type: 'Identifier', name: 'extra' }], 1, 0, 1, 30)
      noUnnecessaryEncodeUriComponentSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(0)
    })
  })
  ...
  describe('edge cases', () => {
    test('should not report null node', () => {
      const { context, reports } = createMockContext()
      noUnnecessaryEncodeUriComponentSpreadRule.create(context).CallExpression!(null)
      expect(reports).toHaveLength(0)
    })

    test('should not report undefined node', () => {
      const { context, reports } = createMockContext()
      noUnnecessaryEncodeUriComponentSpreadRule.create(context).CallExpression!(undefined)
      expect(reports).toHaveLength(0)
    })

    test('should not report non-CallExpression type', () => {
      const { context, reports } = createMockContext()
      noUnnecessaryEncodeUriComponentSpreadRule.create(context).CallExpression!({ type: 'Literal' })
      expect(reports).toHaveLength(0)
    })
    ...
    test('should not report MemberExpression callee', () => {
      ...
      const node = {
        type: 'CallExpression',
        callee: { type: 'MemberExpression', object: { type: 'Identifier', name: 'foo' }, property: { type: 'Identifier', name: 'encodeURIComponent' }, computed: false },
  ...
  describe('location and structure', () => {
    test('should report with correct location line 1', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('encodeURIComponent', [makeSpreadArg({ type: 'Identifier', name: 'items' })], 1, 0, 1, 15)
      noUnnecessaryEncodeUriComponentSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
    ...
    test('should report with correct location line 2', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('encodeURIComponent', [makeSpreadArg({ type: 'Identifier', name: 'items' })], 2, 5, 2, 25)
      noUnnecessaryEncodeUriComponentSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 10', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('encodeURIComponent', [makeSpreadArg({ type: 'Identifier', name: 'items' })], 10, 0, 10, 20)
      noUnnecessaryEncodeUriComponentSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 100', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('encodeURIComponent', [makeSpreadArg({ type: 'Identifier', name: 'items' })], 100, 8, 100, 30)
      noUnnecessaryEncodeUriComponentSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 1', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('encodeURIComponent', [makeSpreadArg({ type: 'Identifier', name: 'items' })], 1, 20, 3, 15)
      noUnnecessaryEncodeUriComponentSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
      expect(reports[0].loc?.start.line).toBe(1)
