...
describe('no-tabs rule', () => {
  ...
  describe('positive cases — reports tab', () => {
    ...
    test('reports for string with middle tab', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral(makeStringNode('hello\tworld'))
      expect(reports.length).toBe(1)
    })
    ...
    test('reports for string with tab between words', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral(makeStringNode('foo\tbar'))
      expect(reports.length).toBe(1)
    })
    ...
    test('reports for string with tab at beginning of indented line', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral(makeStringNode('\t    code'))
      expect(reports.length).toBe(1)
    })
    ...
    test('reports for string with tab in the middle of text', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral(makeStringNode('a\tb\tc'))
      expect(reports.length).toBe(1)
    })
    ...
    test('report message mentions tab character', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral(makeStringNode('a\tb'))
    ...
    test('reports for string with tab after spaces indentation', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral(makeStringNode('    \tindented'))
      expect(reports.length).toBe(1)
    })
    ...
    test('reports for string with tab between numbers', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral(makeStringNode('123\t456'))
      expect(reports.length).toBe(1)
    })
    ...
    test('reports for string with carriage return and tab', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral(makeStringNode('\r\t'))
      expect(reports.length).toBe(1)
    })
    ...
    test('report loc values are preserved from node', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral(makeStringNode('\t', 5, 10, 5, 15))
  ...
  describe('negative cases — does NOT report', () => {
    ...
    test('does not report for UnaryExpression node type', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral({ type: 'UnaryExpression', operator: '!', prefix: true, argument: {}, loc: makeLoc(1, 0, 1, 2) })
      expect(reports.length).toBe(0)
    })
    ...
    test('does not report when value is a number', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral({ type: 'StringLiteral', value: 123, loc: makeLoc(1, 0, 1, 5) })
      expect(reports.length).toBe(0)
    })
    ...
    test('does not report for single char string "a"', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral(makeStringNode('a'))
      expect(reports.length).toBe(0)
    })
    ...
    test('does not report for string with unicode content', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral(makeStringNode('hello 世界'))
      expect(reports.length).toBe(0)
    })
  })
  ...
  describe('edge cases', () => {
    test('separate create() calls have independent state', () => {
      ...
      visitor1.StringLiteral(makeStringNode('\t'))
      visitor2.StringLiteral(makeStringNode('hello'))
    ...
    test('mixed valid/invalid count correctly', () => {
      ...
      visitor.StringLiteral(makeStringNode('hello'))
      visitor.StringLiteral(makeStringNode('\t'))
      visitor.StringLiteral(makeStringNode('world'))
      visitor.StringLiteral(makeStringNode('a\tb'))
      visitor.StringLiteral(makeStringNode('spaces only'))
      expect(reports.length).toBe(2)
    })
    ...
    test('handles node with empty loc object', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral({ type: 'StringLiteral', value: '\t', loc: {} })
      expect(reports.length).toBe(1)
    })
    ...
    test('handles node with partial loc (missing end)', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral({ type: 'StringLiteral', value: '\t', loc: { start: { line: 3, column: 5 } } })
      expect(reports.length).toBe(1)
    ...
    test('handles node with _parent property', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral({ type: 'StringLiteral', value: '\t', loc: makeLoc(1, 0, 1, 5), _parent: {} })
      expect(reports.length).toBe(1)
    })
    ...
    test('report loc reflects specific node location values', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral(makeStringNode('\t', 10, 4, 10, 12))
    ...
    test('does not report when value is an object', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral({ type: 'StringLiteral', value: { toString: () => '\t' }, loc: makeLoc(1, 0, 1, 5) })
      expect(reports.length).toBe(0)
    })
    ...
    test('handles FunctionExpression node type without error', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral({ type: 'FunctionExpression', id: null, params: [], body: { type: 'BlockStatement', body: [] }, loc: makeLoc(1, 0, 1, 20) })
      expect(reports.length).toBe(0)
    })
    ...
    test('handles ArrowFunctionExpression node type without error', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral({ type: 'ArrowFunctionExpression', params: [], body: { type: 'BlockStatement', body: [] }, loc: makeLoc(1, 0, 1, 15) })
      expect(reports.length).toBe(0)
    })
    ...
    test('handles ReturnStatement node type without error', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral({ type: 'ReturnStatement', argument: null, loc: makeLoc(1, 0, 1, 6) })
      expect(reports.length).toBe(0)
    })
    ...
    test('handles IfStatement node type without error', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral({ type: 'IfStatement', test: {}, consequent: {}, loc: makeLoc(1, 0, 1, 15) })
      expect(reports.length).toBe(0)
    })
    ...
    test('handles VariableDeclaration node type without error', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral({ type: 'VariableDeclaration', declarations: [], kind: 'const', loc: makeLoc(1, 0, 1, 10) })
      expect(reports.length).toBe(0)
    })
    ...
    test('handles ExpressionStatement node type without error', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral({ type: 'ExpressionStatement', expression: {}, loc: makeLoc(1, 0, 1, 1) })
      expect(reports.length).toBe(0)
    })
    ...
    test('handles BlockStatement node type without error', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral({ type: 'BlockStatement', body: [], loc: makeLoc(1, 0, 1, 2) })
      expect(reports.length).toBe(0)
    })
    ...
    test('handles ObjectExpression node type without error', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral({ type: 'ObjectExpression', properties: [], loc: makeLoc(1, 0, 1, 2) })
      expect(reports.length).toBe(0)
    })
    ...
    test('handles ArrayExpression node type without error', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral({ type: 'ArrayExpression', elements: [], loc: makeLoc(1, 0, 1, 2) })
      expect(reports.length).toBe(0)
    })
    ...
    test('handles ConditionalExpression node type without error', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral({ type: 'ConditionalExpression', test: {}, consequent: {}, alternate: {}, loc: makeLoc(1, 0, 1, 15) })
      expect(reports.length).toBe(0)
    })
    ...
    test('handles long string with single tab', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral(makeStringNode('this is a very long string with a\ttab in the middle'))
      expect(reports.length).toBe(1)
    })
    ...
    test('handles long string without any tabs', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral(makeStringNode('this is a very long string with no tabs at all'))
      expect(reports.length).toBe(0)
    })
    ...
    test('handles node with string "value" that happens to be "undefined"', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral({ type: 'StringLiteral', value: 'undefined', loc: makeLoc(1, 0, 1, 9) })
      expect(reports.length).toBe(0)
    })
    ...
    test('handles node where value is empty string explicitly', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral({ type: 'StringLiteral', value: '', loc: makeLoc(1, 0, 1, 2) })
      expect(reports.length).toBe(0)
    })
    ...
    test('report loc end values are preserved from node', () => {
      const { context, reports } = createMockContext()
      const visitor = noTabsRule.create(context)
      visitor.StringLiteral(makeStringNode('\t', 2, 8, 4, 20))
