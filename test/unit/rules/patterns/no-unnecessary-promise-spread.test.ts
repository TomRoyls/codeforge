...
import { noUnnecessaryPromiseSpreadRule } from '../../../../src/rules/patterns/no-unnecessary-promise-spread.js'
...
describe('no-unnecessary-promise-spread', () => {

  describe('reports on Promise with single spread argument', () => {
    test('should report Promise(...items)', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Promise', [makeSpreadArg({ type: 'Identifier', name: 'items' })], 1, 0, 1, 20)
      noUnnecessaryPromiseSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toContain('Promise')
    })

    test('should report Promise(...arr)', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Promise', [makeSpreadArg({ type: 'Identifier', name: 'arr' })], 1, 0, 1, 20)
      noUnnecessaryPromiseSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
    })

    test('should report Promise(...[1, 2, 3])', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Promise', [makeSpreadArg({ type: 'ArrayExpression', elements: [] })], 1, 0, 1, 20)
      noUnnecessaryPromiseSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
    })
  })
  ...
  describe('does not report when there is no spread', () => {
    test('should not report Promise(x)', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Promise', [{ type: 'Identifier', name: 'x' }], 1, 0, 1, 10)
      noUnnecessaryPromiseSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(0)
    })

    test('should not report Promise("hello")', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Promise', [{ type: 'Literal', value: 'hello' }], 1, 0, 1, 10)
      noUnnecessaryPromiseSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(0)
    })

    test('should not report Promise()', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Promise', [], 1, 0, 1, 10)
      noUnnecessaryPromiseSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(0)
    })
  })
  ...
  describe('does not report with multiple arguments', () => {
    test('should not report Promise(...items, extra)', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Promise', [makeSpreadArg({ type: 'Identifier', name: 'items' }), { type: 'Identifier', name: 'extra' }], 1, 0, 1, 30)
      noUnnecessaryPromiseSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(0)
    })
  })
  ...
  describe('edge cases', () => {
    test('should not report null node', () => {
      const { context, reports } = createMockContext()
      noUnnecessaryPromiseSpreadRule.create(context).CallExpression!(null)
      expect(reports).toHaveLength(0)
    })

    test('should not report undefined node', () => {
      const { context, reports } = createMockContext()
      noUnnecessaryPromiseSpreadRule.create(context).CallExpression!(undefined)
      expect(reports).toHaveLength(0)
    })

    test('should not report non-CallExpression type', () => {
      const { context, reports } = createMockContext()
      noUnnecessaryPromiseSpreadRule.create(context).CallExpression!({ type: 'Literal' })
      expect(reports).toHaveLength(0)
    })
    ...
    test('should not report MemberExpression callee', () => {
      ...
      const node = {
        type: 'CallExpression',
        callee: { type: 'MemberExpression', object: { type: 'Identifier', name: 'foo' }, property: { type: 'Identifier', name: 'Promise' }, computed: false },
  ...
  describe('location and structure', () => {
    test('should report with correct location line 1', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Promise', [makeSpreadArg({ type: 'Identifier', name: 'items' })], 1, 0, 1, 15)
      noUnnecessaryPromiseSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
    ...
    test('should report with correct location line 2', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Promise', [makeSpreadArg({ type: 'Identifier', name: 'items' })], 2, 5, 2, 25)
      noUnnecessaryPromiseSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 10', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Promise', [makeSpreadArg({ type: 'Identifier', name: 'items' })], 10, 0, 10, 20)
      noUnnecessaryPromiseSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 100', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Promise', [makeSpreadArg({ type: 'Identifier', name: 'items' })], 100, 8, 100, 30)
      noUnnecessaryPromiseSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 1', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Promise', [makeSpreadArg({ type: 'Identifier', name: 'items' })], 1, 20, 3, 15)
      noUnnecessaryPromiseSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
      expect(reports[0].loc?.start.line).toBe(1)
