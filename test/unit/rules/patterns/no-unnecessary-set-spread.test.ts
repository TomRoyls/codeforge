...
import { noUnnecessarySetSpreadRule } from '../../../../src/rules/patterns/no-unnecessary-set-spread.js'
...
describe('no-unnecessary-set-spread', () => {

  describe('reports on Set with single spread argument', () => {
    test('should report Set(...items)', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Set', [makeSpreadArg({ type: 'Identifier', name: 'items' })], 1, 0, 1, 20)
      noUnnecessarySetSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toContain('Set')
    })

    test('should report Set(...arr)', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Set', [makeSpreadArg({ type: 'Identifier', name: 'arr' })], 1, 0, 1, 20)
      noUnnecessarySetSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
    })

    test('should report Set(...[1, 2, 3])', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Set', [makeSpreadArg({ type: 'ArrayExpression', elements: [] })], 1, 0, 1, 20)
      noUnnecessarySetSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
    })
  })
  ...
  describe('does not report when there is no spread', () => {
    test('should not report Set(x)', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Set', [{ type: 'Identifier', name: 'x' }], 1, 0, 1, 10)
      noUnnecessarySetSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(0)
    })

    test('should not report Set("hello")', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Set', [{ type: 'Literal', value: 'hello' }], 1, 0, 1, 10)
      noUnnecessarySetSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(0)
    })

    test('should not report Set()', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Set', [], 1, 0, 1, 10)
      noUnnecessarySetSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(0)
    })
  })
  ...
  describe('does not report with multiple arguments', () => {
    test('should not report Set(...items, extra)', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Set', [makeSpreadArg({ type: 'Identifier', name: 'items' }), { type: 'Identifier', name: 'extra' }], 1, 0, 1, 30)
      noUnnecessarySetSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(0)
    })
  })
  ...
  describe('edge cases', () => {
    test('should not report null node', () => {
      const { context, reports } = createMockContext()
      noUnnecessarySetSpreadRule.create(context).CallExpression!(null)
      expect(reports).toHaveLength(0)
    })

    test('should not report undefined node', () => {
      const { context, reports } = createMockContext()
      noUnnecessarySetSpreadRule.create(context).CallExpression!(undefined)
      expect(reports).toHaveLength(0)
    })

    test('should not report non-CallExpression type', () => {
      const { context, reports } = createMockContext()
      noUnnecessarySetSpreadRule.create(context).CallExpression!({ type: 'Literal' })
      expect(reports).toHaveLength(0)
    })
    ...
    test('should not report MemberExpression callee', () => {
      ...
      const node = {
        type: 'CallExpression',
        callee: { type: 'MemberExpression', object: { type: 'Identifier', name: 'foo' }, property: { type: 'Identifier', name: 'Set' }, computed: false },
  ...
  describe('location and structure', () => {
    test('should report with correct location line 1', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Set', [makeSpreadArg({ type: 'Identifier', name: 'items' })], 1, 0, 1, 15)
      noUnnecessarySetSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
    ...
    test('should report with correct location line 2', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Set', [makeSpreadArg({ type: 'Identifier', name: 'items' })], 2, 5, 2, 25)
      noUnnecessarySetSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 10', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Set', [makeSpreadArg({ type: 'Identifier', name: 'items' })], 10, 0, 10, 20)
      noUnnecessarySetSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 100', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Set', [makeSpreadArg({ type: 'Identifier', name: 'items' })], 100, 8, 100, 30)
      noUnnecessarySetSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 1', () => {
      const { context, reports } = createMockContext()
      const node = makeCall('Set', [makeSpreadArg({ type: 'Identifier', name: 'items' })], 1, 20, 3, 15)
      noUnnecessarySetSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
      expect(reports[0].loc?.start.line).toBe(1)
