...
describe('no-unnecessary-array-includes-spread rule', () => {
  describe('meta', () => {
    ...
    test('should have docs url', () => {
      expect(noUnnecessaryArrayIncludesSpreadRule.meta.docs.url).toBeDefined()
    })
    test('should have description', () => {
      expect(noUnnecessaryArrayIncludesSpreadRule.meta.docs.description).toBeDefined()
    })
    test('should have valid docs description type', () => {
      expect(typeof noUnnecessaryArrayIncludesSpreadRule.meta.docs.description).toBe('string')
    })
  })
  ...
  describe('edge cases', () => {
    test('should not report on empty arguments', () => {
      ...
      const node = makearrIncludesCall([])
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(0)
    })
    test('should not report on two regular arguments', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([{ type: 'Literal', value: 1 }, { type: 'Literal', value: 2 }])
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(0)
    })
  })
  ...
  describe('should report arr.includes(...items) with single spread', () => {
    test('should report arr.includes(...items) case 1', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'items' })], 1, 0, 1, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...arr) case 2', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'arr' })], 2, 0, 2, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...args) case 3', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'args' })], 3, 0, 3, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...list) case 4', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'list' })], 4, 0, 4, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...data) case 5', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'data' })], 5, 0, 5, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...values) case 6', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'values' })], 6, 0, 6, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...nums) case 7', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'nums' })], 7, 0, 7, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...rest) case 8', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'rest' })], 8, 0, 8, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...options) case 9', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'options' })], 9, 0, 9, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...params) case 10', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'params' })], 10, 0, 10, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...collection) case 11', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'collection' })], 11, 0, 11, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...elements) case 12', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'elements' })], 12, 0, 12, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...entries) case 13', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'entries' })], 13, 0, 13, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...objs) case 14', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'objs' })], 14, 0, 14, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...source) case 15', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'source' })], 15, 0, 15, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...input) case 16', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'input' })], 16, 0, 16, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...payload) case 17', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'payload' })], 17, 0, 17, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...buffer) case 18', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'buffer' })], 18, 0, 18, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...chunk) case 19', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'chunk' })], 19, 0, 19, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...segment) case 20', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'segment' })], 20, 0, 20, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...portion) case 21', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'portion' })], 21, 0, 21, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...range) case 22', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'range' })], 22, 0, 22, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...tuple) case 23', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'tuple' })], 23, 0, 23, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...seq) case 24', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'seq' })], 24, 0, 24, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...iter) case 25', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'iter' })], 25, 0, 25, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...result) case 26', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'result' })], 26, 0, 26, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...output) case 27', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'output' })], 27, 0, 27, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...response) case 28', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'response' })], 28, 0, 28, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...records) case 29', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'records' })], 29, 0, 29, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...rows) case 30', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'rows' })], 30, 0, 30, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...cols) case 31', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'cols' })], 31, 0, 31, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...cells) case 32', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'cells' })], 32, 0, 32, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...fields) case 33', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'fields' })], 33, 0, 33, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...props) case 34', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'props' })], 34, 0, 34, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...attrs) case 35', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'attrs' })], 35, 0, 35, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...keys) case 36', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'keys' })], 36, 0, 36, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...vals) case 37', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'vals' })], 37, 0, 37, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...pairs) case 38', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'pairs' })], 38, 0, 38, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...nodes) case 39', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'nodes' })], 39, 0, 39, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
    test('should report arr.includes(...items2) case 40', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'items2' })], 40, 0, 40, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].message).toBeDefined()
    })
  ...
  describe('location and structure', () => {
    test('should report with correct location line 2', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'items' })], 2, 5, 2, 30)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
    ...
    test('should report with correct location line 3', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'items' })], 3, 10, 3, 35)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 5', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'items' })], 5, 0, 5, 20)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 10', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'items' })], 10, 8, 10, 28)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 15', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'items' })], 15, 3, 15, 23)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 20', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'items' })], 20, 0, 20, 15)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 25', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'items' })], 25, 12, 25, 37)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 30', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'items' })], 30, 1, 30, 21)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 40', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'items' })], 40, 5, 40, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 50', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'items' })], 50, 0, 50, 30)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 60', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'items' })], 60, 7, 60, 27)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 70', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'items' })], 70, 2, 70, 22)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 80', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'items' })], 80, 0, 80, 20)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 90', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'items' })], 90, 15, 90, 40)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 100', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'items' })], 100, 0, 100, 25)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 150', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'items' })], 150, 3, 150, 23)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
    ...
    test('should report with correct location line 200', () => {
      const { context, reports } = createMockContext()
      const node = makearrIncludesCall([makeSpreadArg({ type: 'Identifier', name: 'items' })], 200, 8, 200, 33)
      noUnnecessaryArrayIncludesSpreadRule.create(context).CallExpression!(node)
      expect(reports).toHaveLength(1)
      expect(reports[0].loc).toBeDefined()
