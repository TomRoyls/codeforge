...
import { noUnnecessaryArrayMapSpreadRule } from '../../../../src/rules/patterns/index.js'
...
describe('no-unnecessary-array-map-spread rule', () => {
  describe('meta', () => {
    ...
    test('should have a description', () => {
      expect(noUnnecessaryArrayMapSpreadRule.meta.docs?.description).toBeTruthy()
    })

    test('should have description mentioning map and spread', () => {
      const desc = noUnnecessaryArrayMapSpreadRule.meta.docs?.description?.toLowerCase() ?? ''
      expect(desc).toMatch(/map/)
      expect(desc).toMatch(/spread/)
    })

    test('should have correct docs URL', () => {
      expect(noUnnecessaryArrayMapSpreadRule.meta.docs?.url).toBe(
        'https://github.com/nickelser/codeforge/blob/main/src/rules/patterns/no-unnecessary-array-map-spread.ts',
      )
    })
  ...
  describe('structure', () => {
    test('create() returns visitor with CallExpression', () => {
      ...
      const visitor = noUnnecessaryArrayMapSpreadRule.create(context)
      expect(visitor).toHaveProperty('CallExpression')
      expect(typeof visitor.CallExpression).toBe('function')
    })

    test('default export matches named export', () => {
      expect(noUnnecessaryArrayMapSpreadRule).toBeDefined()
      expect(noUnnecessaryArrayMapSpreadRule.meta).toBeDefined()
      expect(noUnnecessaryArrayMapSpreadRule.create).toBeDefined()
    })
  })

  // ===== POSITIVE CASES — REPORTS (27) =====

  describe('positive cases — reports arr.map(...spread)', () => {
    test('reports for arr.map(...items) with Identifier argument', () => {
      const { context, reports } = createMockContext()
      const visitor = noUnnecessaryArrayMapSpreadRule.create(context)
      visitor.CallExpression(makeMapSpreadCall({ type: 'Identifier', name: 'arr' }))
      expect(reports.length).toBe(1)
    })

    test('reports for [].map(...items) with ArrayExpression callee', () => {
      const { context, reports } = createMockContext()
      const visitor = noUnnecessaryArrayMapSpreadRule.create(context)
      visitor.CallExpression(makeMapSpreadCall({ type: 'ArrayExpression', elements: [] }))
      expect(reports.length).toBe(1)
    })

    test('reports for obj.arr.map(...items) with MemberExpression callee', () => {
      const { context, reports } = createMockContext()
      const visitor = noUnnecessaryArrayMapSpreadRule.create(context)
    ...
    test('reports for getArr().map(...items) with CallExpression callee', () => {
      const { context, reports } = createMockContext()
      const visitor = noUnnecessaryArrayMapSpreadRule.create(context)
      visitor.CallExpression(
        makeMapSpreadCall({
    ...
    test('reports for SpreadElement with MemberExpression argument ...obj.items', () => {
      const { context, reports } = createMockContext()
      const visitor = noUnnecessaryArrayMapSpreadRule.create(context)
      visitor.CallExpression(
        makeMapSpreadCall(
    ...
    test('reports for SpreadElement with CallExpression argument ...getItems()', () => {
      const { context, reports } = createMockContext()
      const visitor = noUnnecessaryArrayMapSpreadRule.create(context)
      visitor.CallExpression(
        makeMapSpreadCall(
          { type: 'Identifier', name: 'arr' },
          { type: 'CallExpression', callee: { type: 'Identifier', name: 'getItems' }, arguments: [] },
        ),
      )
      expect(reports.length).toBe(1)
    })

    test('reports for SpreadElement with ArrayExpression argument ...[1, 2, 3]', () => {
      const { context, reports } = createMockContext()
      const visitor = noUnnecessaryArrayMapSpreadRule.create(context)
      visitor.CallExpression(
        makeMapSpreadCall(
    ...
    test('report message is exactly as defined in rule source', () => {
      ...
      expect(reports[0].message).toBe(
        'arr.map(...items) with spread is unusual. map() expects a callback function.',
      )
    })
    ...
    test('report node matches the input CallExpression node', () => {
      const { context, reports } = createMockContext()
      const visitor = noUnnecessaryArrayMapSpreadRule.create(context)
      const node = makeMapSpreadCall({ type: 'Identifier', name: 'arr' })
    ...
    test('report loc values are preserved from node', () => {
      const { context, reports } = createMockContext()
      const visitor = noUnnecessaryArrayMapSpreadRule.create(context)
      visitor.CallExpression(makeMapSpreadCall({ type: 'Identifier', name: 'arr' }, undefined, 5, 10, 5, 30))
    ...
    test('reports for SpreadElement with NewExpression argument', () => {
      ...
      visitor.CallExpression(
        makeMapSpreadCall(
          { type: 'Identifier', name: 'arr' },
          { type: 'NewExpression', callee: { type: 'Identifier', name: 'Set' }, arguments: [] },
        ),
      )
      expect(reports.length).toBe(1)
    })
    ...
    test('reports for SpreadElement with Literal argument ...42', () => {
      const { context, reports } = createMockContext()
      const visitor = noUnnecessaryArrayMapSpreadRule.create(context)
      visitor.CallExpression(
        makeMapSpreadCall(
    ...
    test('reports for chained call arr.map(fn).map(...items)', () => {
      const { context, reports } = createMockContext()
      const visitor = noUnnecessaryArrayMapSpreadRule.create(context)
      const innerCall = {
        type: 'CallExpression',
  ...
  describe('negative cases — does NOT report', () => {
    test('does not report for arr.map(x => x) — arrow function argument', () => {
      const { context, reports } = createMockContext()
      const visitor = noUnnecessaryArrayMapSpreadRule.create(context)
      visitor.CallExpression({
        type: 'CallExpression',
    ...
    test('does not report for arr.map(function(x) { return x; }) — FunctionExpression argument', () => {
      const { context, reports } = createMockContext()
      const visitor = noUnnecessaryArrayMapSpreadRule.create(context)
      visitor.CallExpression({
        type: 'CallExpression',
    ...
    test('does not report for arr.map(fn, thisArg) — two arguments', () => {
      const { context, reports } = createMockContext()
      const visitor = noUnnecessaryArrayMapSpreadRule.create(context)
      visitor.CallExpression({
        type: 'CallExpression',
        ...
        arguments: [{ type: 'Identifier', name: 'fn' }, { type: 'Identifier', name: 'thisArg' }],
        loc: makeLoc(1, 0, 1, 25),
      })
      expect(reports.length).toBe(0)
    })

    test('does not report for arr.map(fn, thisArg, extra) — three arguments', () => {
      const { context, reports } = createMockContext()
      const visitor = noUnnecessaryArrayMapSpreadRule.create(context)
      visitor.CallExpression({
        type: 'CallExpression',
        ...
        arguments: [{ type: 'Identifier', name: 'fn' }, { type: 'Identifier', name: 'thisArg' }, { type: 'Literal', value: 42 }],
    ...
    test('does not report for computed member expression arr["map"](...items)', () => {
      const { context, reports } = createMockContext()
      const visitor = noUnnecessaryArrayMapSpreadRule.create(context)
      visitor.CallExpression({
        type: 'CallExpression',
    ...
    test('does not report when property name is "Map" (uppercase M)', () => {
      const { context, reports } = createMockContext()
      const visitor = noUnnecessaryArrayMapSpreadRule.create(context)
      visitor.CallExpression({
        type: 'CallExpression',
    ...
    test('does not report when callee is not MemberExpression — plain Identifier', () => {
      const { context, reports } = createMockContext()
      const visitor = noUnnecessaryArrayMapSpreadRule.create(context)
      visitor.CallExpression({
        type: 'CallExpression',
  ...
  describe('edge cases', () => {
    test('separate create() calls have independent state', () => {
      ...
      const visitor1 = noUnnecessaryArrayMapSpreadRule.create(ctx1)
      const visitor2 = noUnnecessaryArrayMapSpreadRule.create(ctx2)
      visitor1.CallExpression(makeMapSpreadCall({ type: 'Identifier', name: 'arr' }))
      visitor2.CallExpression({
        ...
        arguments: [{ type: 'ArrowFunctionExpression', params: [], body: { type: 'Identifier', name: 'x' } }],
        loc: makeLoc(1, 0, 1, 20),
      })
    ...
    test('create returns a new visitor each call', () => {
      const { context } = createMockContext()
      const visitor1 = noUnnecessaryArrayMapSpreadRule.create(context)
      const visitor2 = noUnnecessaryArrayMapSpreadRule.create(context)
      expect(visitor1).not.toBe(visitor2)
    })

    test('meta is same reference across multiple accesses', () => {
      const meta1 = noUnnecessaryArrayMapSpreadRule.meta
      const meta2 = noUnnecessaryArrayMapSpreadRule.meta
    ...
    test('rule exports are correct', () => {
      expect(noUnnecessaryArrayMapSpreadRule).toBeDefined()
      expect(typeof noUnnecessaryArrayMapSpreadRule.create).toBe('function')
      expect(typeof noUnnecessaryArrayMapSpreadRule.meta).toBe('object')
    })
    ...
    test('report loc reflects specific node location values', () => {
      const { context, reports } = createMockContext()
      const visitor = noUnnecessaryArrayMapSpreadRule.create(context)
      visitor.CallExpression(makeMapSpreadCall({ type: 'Identifier', name: 'arr' }, undefined, 10, 4, 10, 25))
    ...
    test('computed: false does not prevent reporting', () => {
      const { context, reports } = createMockContext()
      const visitor = noUnnecessaryArrayMapSpreadRule.create(context)
      visitor.CallExpression({
        type: 'CallExpression',
    ...
    test('computed: true does prevent reporting even with correct method name', () => {
      const { context, reports } = createMockContext()
      const visitor = noUnnecessaryArrayMapSpreadRule.create(context)
      visitor.CallExpression({
        type: 'CallExpression',
    ...
    test('mixed valid/invalid count correctly', () => {
      ...
      // positive: [].map(...items)
      visitor.CallExpression(makeMapSpreadCall({ type: 'ArrayExpression', elements: [] }))
      // negative: arr.filter(...items) — wrong method
      visitor.CallExpression({
        type: 'CallExpression',
        callee: {
          type: 'MemberExpression',
      ...
      // positive: getArr().map(...data)
      visitor.CallExpression(makeMapSpreadCall({ type: 'CallExpression', callee: { type: 'Identifier', name: 'getArr' }, arguments: [] }))
      expect(reports.length).toBe(3)
    })
