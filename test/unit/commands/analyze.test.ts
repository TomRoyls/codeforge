...
vi.mock('../../../src/core/reporter.js', () => ({
  Reporter: vi.fn().mockImplementation(() => ({ writeReport: vi.fn().mockResolvedValue(undefined) })),
}));
...
vi.mock('../../../src/config/discovery.js', () => ({ findConfigPath: vi.fn().mockResolvedValue(null) }));
...
vi.mock('../../../src/config/merger.js', () => ({ mergeConfigs: vi.fn((base, cli) => ({ ...base, ...cli })) }));
...
describe('Analyze Command', () => {
  ...
  beforeEach(async () => {
    ...
    mockDiscoverFiles = (await import('../../../src/core/file-discovery.js')).discoverFiles as ReturnType<typeof vi.fn>;
    mockTraverseAST = (await import('../../../src/ast/visitor.js')).traverseAST as ReturnType<typeof vi.fn>;
  ...
  describe('Command metadata', () => {
    ...
    test('format flag has correct options', () => {
      expect(Analyze.flags.format.options).toEqual(['console', 'html', 'json', 'junit']);
    });
  ...
  describe('Flag characters', () => {
    test('files flag has char f', () => { expect(Analyze.flags.files.char).toBe('f'); });
    test('ignore flag has char i', () => { expect(Analyze.flags.ignore.char).toBe('i'); });
    test('rules flag has char r', () => { expect(Analyze.flags.rules.char).toBe('r'); });
    test('config flag has char c', () => { expect(Analyze.flags.config.char).toBe('c'); });
    test('output flag has char o', () => { expect(Analyze.flags.output.char).toBe('o'); });
    test('quiet flag has char q', () => { expect(Analyze.flags.quiet.char).toBe('q'); });
    test('verbose flag has char v', () => { expect(Analyze.flags.verbose.char).toBe('v'); });
  });

  describe('Multiple value flags', () => {
    test('files flag supports multiple', () => { expect(Analyze.flags.files.multiple).toBe(true); });
    test('ignore flag supports multiple', () => { expect(Analyze.flags.ignore.multiple).toBe(true); });
    test('rules flag supports multiple', () => { expect(Analyze.flags.rules.multiple).toBe(true); });
  });
  ...
  describe('Args configuration', () => {
    ...
    test('path arg is not required', () => { expect(Analyze.args.path.required).toBe(false); });
    test('path arg defaults to dot', () => { expect(Analyze.args.path.default).toBe('.'); });
  });
  ...
  describe('catch error handling', () => {
    test('handles CLIError with suggestions', async () => {
      ...
      cmdWithMock.error = vi.fn(() => { throw new Error('cmd'); });
  ...
  describe('run integration', () => {
    ...
    test('calls discoverFiles', async () => {
      ...
      const cmd = createCommandWithMockedParse({
        files: [], ignore: [], format: 'console', quiet: true, verbose: false, 'fail-on-warnings': false,
      });
      (cmd as { exit: (c: number) => never }).exit = (code: number) => { throw new ExitCodeError(code); };
      try { await cmd.run(); } catch { /* empty */ }
    ...
    test('calls Reporter with json format', async () => {
      ...
      mockDiscoverFiles.mockResolvedValue([createMockFile('test.ts')]);
      const cmd = createCommandWithMockedParse({
        files: [], ignore: [], format: 'json', output: '/out.json', quiet: true, verbose: false, 'fail-on-warnings': false,
      });
      (cmd as { exit: (c: number) => never }).exit = (code: number) => { throw new ExitCodeError(code); };
      try { await cmd.run(); } catch { /* empty */ }
      expect(Reporter).toHaveBeenCalledWith(expect.objectContaining({ format: 'json', outputPath: '/out.json' }));
    });
    ...
    test('sets DEBUG log level when verbose', async () => {
      ...
      mockDiscoverFiles.mockResolvedValue([]);
      const cmd = createCommandWithMockedParse({
        files: [], ignore: [], format: 'console', quiet: false, verbose: true, 'fail-on-warnings': false,
      });
      (cmd as { exit: (c: number) => never }).exit = (code: number) => { throw new ExitCodeError(code); };
      try { await cmd.run(); } catch { /* empty */ }
    ...
    test('exits with 0 when no files found', async () => {
      mockDiscoverFiles.mockResolvedValue([]);
      const cmd = createCommandWithMockedParse({
        files: [], ignore: [], format: 'console', quiet: false, verbose: false, 'fail-on-warnings': false,
      });
      let exitCode = 0;
    ...
    test('handles multiple files', async () => {
      mockDiscoverFiles.mockResolvedValue([createMockFile('a.ts'), createMockFile('b.ts'), createMockFile('c.ts')]);
      const cmd = createCommandWithMockedParse({
        files: [], ignore: [], format: 'console', quiet: true, verbose: false, 'fail-on-warnings': false,
      });
      let exitCode = 0;
    ...
    test('counts violations by severity', async () => {
      ...
      (Reporter as ReturnType<typeof vi.fn>).mockImplementation(() => ({
        writeReport: vi.fn().mockImplementation((d: typeof data) => { data = d; return Promise.resolve(); }),
      }));
      const cmd = createCommandWithMockedParse({
        files: [], ignore: [], format: 'json', quiet: true, verbose: false, 'fail-on-warnings': false,
      });
      (cmd as { exit: (c: number) => never }).exit = (code: number) => { throw new ExitCodeError(code); };
      try { await cmd.run(); } catch { /* empty */ }
