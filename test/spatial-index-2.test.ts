...
describe('SpatialIndex2', () => {
  ...
  it('should handle remove then isEmpty', () => {
    const index = new SpatialIndex2()
    index.insert('a', 0, 0)
    index.remove('a')
    expect(index.isEmpty()).toBe(true)
  ...
  it('should handle toArray on populated index', () => {
    const index = new SpatialIndex2<string>()
    index.insert('a', 0, 0)
    index.insert('b', 10, 10)
    expect(index.toArray()).toHaveLength(2)
  })
})
  it('should handle get method', () => {
    const index = new SpatialIndex2()
    index.insert('p1', 5, 5)
    const pt = index.get('p1')
    expect(pt).toBeDefined()
    expect(pt!.x).toBe(5)
    expect(pt!.y).toBe(5)
  })
